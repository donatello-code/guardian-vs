# Guardian Hooks Documentation

## Overview

Guardian hooks allow you to execute custom scripts at specific points in the agentic workflow. Hooks can be placed in either:
- **Global hooks directory**: `~/Documents/Guardian/Hooks/` (applies to all workspaces)
- **Workspace hooks directory**: `.guardianrules/hooks/` (applies to the workspace the repo is part of)

Hooks run automatically when enabled.

## Enabling Hooks

1. Open Guardian settings in VSCode
...
- **Global Location**: `~/Documents/Guardian/Hooks/TaskStart`
- **Workspace Location**: `.guardianrules/hooks/TaskStart`
...
- **Global Location**: `~/Documents/Guardian/Hooks/TaskResume`
- **Workspace Location**: `.guardianrules/hooks/TaskResume`
...
- **Global Location**: `~/Documents/Guardian/Hooks/TaskCancel`
- **Workspace Location**: `.guardianrules/hooks/TaskCancel`
...
- **Global Location**: `~/Documents/Guardian/Hooks/TaskComplete`
- **Workspace Location**: `.guardianrules/hooks/TaskComplete`
...
- **Global Location**: `~/Documents/Guardian/Hooks/UserPromptSubmit`
- **Workspace Location**: `.guardianrules/hooks/UserPromptSubmit`

### PreToolUse Hook
- **When**: Runs BEFORE a tool is executed
- **Purpose**: Validate parameters, block execution, or add context
- **Global Location**: `~/Documents/Guardian/Hooks/PreToolUse`
- **Workspace Location**: `.guardianrules/hooks/PreToolUse`

### PostToolUse Hook
- **When**: Runs AFTER a tool completes
- **Purpose**: Observe results, track patterns, or add context
- **Global Location**: `~/Documents/Guardian/Hooks/PostToolUse`
- **Workspace Location**: `.guardianrules/hooks/PostToolUse`
...
- **Global Location**: `~/Documents/Guardian/Hooks/PreCompact`
- **Workspace Location**: `.guardianrules/hooks/PreCompact`

## Cross-Platform Hook Format

Guardian uses a git-style approach for hooks that works consistently across all platforms:
...
Like git hooks, Guardian executes hook files through a shell that interprets the shebang line:
...
```bash
# Create hook file
nano ~/Documents/Guardian/Hooks/PreToolUse

# Make executable
chmod +x ~/Documents/Guardian/Hooks/PreToolUse
```
...
# Log to file
echo "$input" >> ~/.guardian/hook-logs/tool-usage.jsonl
...
Guardian supports two levels of hooks:

### Global Hooks
- **Location**: `~/Documents/Guardian/Hooks/` (macOS/Linux)
...
### Workspace Hooks
- **Location**: `.guardianrules/hooks/` in each workspace root
...
1. The global hooks directory is automatically created at:
   - macOS/Linux: `~/Documents/Guardian/Hooks/`
...
3. Enable hooks in Guardian settings
...
```bash
#!/usr/bin/env bash
# ~/Documents/Guardian/Hooks/PreToolUse
...
```bash
#!/usr/bin/env bash
# .guardianrules/hooks/PreToolUse
...
If you have multiple workspace roots, you can place hooks in each root's `.guardianrules/hooks/` directory. All hooks (global and workspace) may execute concurrently. Their results will be combined:
...
- Look for errors in VSCode's Output panel (Guardian channel)
