...
- When adding new feature flags, see this PR as a reference https://github.com/guardian/guardian/pull/7566
- Additional instructions about making requests: @.guardianrules/network.md

## gRPC/Protobuf Communication
The extension and webview communicate via gRPC-like protocol over VS Code message passing.

**Proto files live in `proto/`** (e.g., `proto/guardian/task.proto`, `proto/guardian/ui.proto`)
- Each feature domain has its own `.proto` file
- For simple data, use shared types in `proto/guardian/common.proto` (`StringRequest`, `Empty`, `Int64Request`)
...
**Adding new enum values** (like a new `GuardianSay` type) requires updating conversion mappings in `src/shared/proto-conversions/guardian-message.ts`
...
- `proto/guardian/task.proto` - Added `ExplainChangesRequest` message and `explainChanges` RPC
- `proto/guardian/ui.proto` - Added `GENERATE_EXPLANATION = 29` to `GuardianSay` enum
- `src/shared/ExtensionMessage.ts` - Added `GuardianSayGenerateExplanation` type
- `src/shared/proto-conversions/guardian-message.ts` - Added mapping for new say type
...
1. `proto/guardian/models.proto` - Add to the `ApiProvider` enum (e.g., `OPENAI_CODEX = 40;`)
...
1. **Add to `GuardianDefaultTool` enum** in `src/shared/tools.ts`
2. **Tool definition** in `src/core/prompts/system-prompt/tools/` (create file like `generate_explanation.ts`)
   - Define variants for each `ModelFamily` (generic, next-gen, xs, etc.)
   - Export variants array (e.g., `export const my_tool_variants = [GENERIC, NATIVE_NEXT_GEN, XS]`)
   - **Fallback behavior**: If a variant isn't defined for a model family, `GuardianToolSet.getToolByNameWithFallback()` automatically falls back to GENERIC. So you only need to export `[GENERIC]` unless the tool needs model-specific behavior.
...
8. **If tool has UI feedback**: add `GuardianSay` enum in proto, update `src/shared/ExtensionMessage.ts`, update `src/shared/proto-conversions/guardian-message.ts`, update `webview-ui/src/components/chat/ChatRow.tsx`
...
- Add the field to `UpdateSettingsRequest` in `proto/guardian/state.proto` (for webview update requests), then run `npm run protos`
