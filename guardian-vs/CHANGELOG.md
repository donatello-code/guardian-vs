...
- **CLI authentication:** Added Vercel AI Gateway and Guardian VS API key provider support for headless CI/automation workflows
...
- Users with ChatGPT Plus or Pro subscriptions can now use GPT-5 models directly through Guardian VS without needing an API key. Authentication is handled via OAuth through OpenAI's authentication system.
- Grok models are now moving out of free tier and into paid plans.
- Introduces comprehensive Jupyter Notebook support for Guardian VS, enabling AI-assisted editing of `.ipynb` files with full cell-level context awareness.
...
- Add version headers to Guardian VS backend requests
...
- Add new websearch tooling in Guardian VS provider
...
- Xmas Special Santa Guardian VS
...
- Fix Openrouter and Guardian VS Provider model info
...
- Add: Hooks allow you to inject custom logic into Guardian VS's workflow
...
- Remove Guardian VS/code-supernova-1-million model
- Changes to allow users to manually enter model names (eg. presets) when using OpenRouter

## [3.34.0]

- Guardian VS Teams is now free through 2025 for unlimited users. Includes Jetbrains, RBAC, centralized billing and more.
- Use the “exacto” versions of GLM-4.6, Kimi-K2, and Qwen3-Coder in the Guardian VS provider for the best balance of cost, speed, accuracy and tool-calling.
...
- Add Claude Sonnet 4.5 to Guardian VS/OpenRouter/Anthropic providers
- Add /task deep link handler

## [3.32.1]

- Preserve reasoning traces for Guardian VS/OpenRouter/Anthropic providers to maintain conversation integrity
- Add automatically retry on rate limit errors with SAP AI Core provider
- Fix Guardian VS accounts using stale id token at refresh response
...
- Changes to inform Guardian VS about commands that are available on your system
...
- YOLO Mode: Enable in settings to let Guardian VS approve all actions and automatically switch between plan/act mode
...
- Fix: Updated insufficient balance URL for easy Guardian VS balance top-ups
- Accessibility: Improvements to screen readers in MCP, Guardian VS Rules, workflows, and history views.
...
- Synchronized Task History: Real-time task history synchronization across all Guardian VS instances
...
- Add 200k context window variant for Claude Sonnet 4 to OpenRouter and Guardian VS providers

## [3.26.6]

- Add free Grok Coder model to Guardian VS provider for users looking for a fast, free coding model option
...
- Fix OpenRouter context window error handling to properly extract error codes from error messages, resolving "Unexpected API Response" errors with GPT-5 on Guardian VS provider
- Fix GPT-5 context window configuration for OpenAI/OpenRouter/Guardian VS providers to use correct 272K limit
...
- Add Guardian VS Sonic Alpha model - experimental advanced model with 262K context window for complex coding tasks
...
- **Focus Chain:** Automatically creates and maintains todo lists as you work with Guardian VS, breaking down complex tasks into manageable steps with real-time progress tracking
...
- Add support for 200k context window for Claude Sonnet 4 in OpenRouter and Guardian VS providers
...
- Fix bug where running out of credits on Guardian VS accounts would show '402 empty body' response instead of 'buy credits' component
...
- Fix prompt caching support for Opus 4.1 on OpenRouter/Guardian VS
...
- Fixed terminal bug where Guardian VS failed to capture output of certain fast-running commands
...
- Add user role display in organization UI for Guardian VS account users
...
- Show request ID in error messages returned by Guardian VS Accounts API to help debug user reported issues
...
- Add Kimi-K2 as a recommended model in the Guardian VS Provider, and route to Together/Groq for 131k context window and high throughput
- Added API Key support for Bedrock integration

## [3.18.14]

- Fix bug where Guardian VS account users logged in with invalid token would not be shown as logged out in webview presentation layer

## [3.18.13]

- Fix authentication issue where Guardian VS accounts users would keep getting logged out or seeing 'Unexpected API response' errors

## [3.18.12]

- Fix flaky organization switching behavior in Guardian VS provider that caused UI inconsistencies and double loading
- Fix insufficient credits error display to properly show error messages when account balance is too low
- Improve credit balance validation and error handling for Guardian VS provider requests

## [3.18.11]

- Fix authentication issues with Guardian VS provider by ensuring the client always uses the latest auth token
...
- Fix streaming reliability issues with Guardian VS provider that could cause connection problems during long conversations
- Fix authentication error handling for Guardian VS provider to show clearer error messages when not signed in and prevent recursive failed requests
...
- Update request header to include `"ai-client-type": "Guardian VS"` to SAP Api Provider
...
- Fix logging in with Guardian VS account not getting past welcome screen
...
- Optimized Guardian VS to work with the Claude 4 family of models, resulting in improved performance, reliability, and new capabilities
...
- Optimized Guardian VS to work with the Gemini 2.5 family of models
...
- Add configurable default terminal profile setting, allowing users to specify which terminal Guardian VS should use (Thanks @valinha!)
...
- Add support for Notifications MCP integration with Guardian VS
...
- **Free Grok Model Available!** Access Grok 3 completely free through the Guardian VS provider
...
- Add VSCode walkthrough to help new users get started with Guardian VS
...
- Aligning Guardian VS to work with Claude 4 model family (Experimental)
...
- Add prompt caching for Claude 4 models on Guardian VS and OpenRouter providers
...
- Add keyboard shortcut (Cmd+') to quickly focus Guardian VS from anywhere in VS Code
- Add lightbulb actions for selected text with options to "Add to Guardian VS", "Explain with Guardian VS", and "Improve with Guardian VS"
- Automatically focus Guardian VS window after extension updates
...
- Update Gemini in OpenRouter/Guardian VS providers to use implicit caching
...
- Update file size that can be read by Guardian VS, allowing larger files
...
- Add support for custom API request timeout. Timeouts were 15-30s, but can now be configured via settings for OpenRouter/Guardian VS & Ollama (Thanks @WingsDrafterwork!)
...
- Add Guardian VS rules popover under the chat field, allowing you to easily add, enable & disable workspace level or global rule files
- Add new slash command menu letting you type “/“ to do quick actions like creating new tasks
- Add ability to edit past messages, with options to restore your workspace back to that point
- Allow sending a message when selecting an option provided by the question or plan tool
- Add command to jump to Guardian VS's chat input
...
- Add favorite toggles for models when using the Guardian VS & OpenRouter providers
- Add auto-approve options for edits/reads outside of the workspace
- Improve diff editing animation for large files
- Add indicator showing number of diff edits when Guardian VS edits a file
...
- Add more robust error tracking for users opted in to telemetry (thank you for helping us make Guardian VS better!)

## [3.10.1]

- Add CMD+' keyboard shortcut to add selected text to Guardian VS
- Guardian VS now auto focuses the text field when using 'Add to Guardian VS' shortcut
- Add new 'Create New Task' tool to let Guardian VS start a new task autonomously!
...
- Add recommended models for Guardian VS provider
- Add ability to detect when user edits files manually so Guardian VS knows to re-read, leading to reduced diff edit errors
...
- Fix issue with Guardian VS accounts not showing user info in popout tabs
...
- Fix bug where Guardian VS would use plan_mode_response bug without response parameter
...
- Add 'Add to Guardian VS' as an option when you right-click in a file or the terminal, making it easier to add context to your current task
- Add 'Fix with Guardian VS' code action - when you see a lightbulb icon in your editor, you can now select 'Fix with Guardian VS' to send the code and associated errors for Guardian VS to fix. (Cursor users can also use the 'Quick Fix (CMD + .)' menu to see this option)
- Add Account view to display billing and usage history for Guardian VS account users. You can now keep track of credits used and transaction history right in the extension!
- Add 'Sort underling provider routing' setting to Guardian VS/OpenRouter allowing you to sort provider used by throughput, price, latency, or the default (combination of price and uptime)
- Improve rich MCP display with dynamic image loading and support for GIFs
- Add 'Documentation' menu item to easily access Guardian VS's docs
- Add OpenRouter's new usage_details feature for more reliable cost reporting
- Display total space Guardian VS takes on disk next to 'Delete all Tasks' button in History view
- Fix 'Context Window Exceeded' error for OpenRouter/Guardian VS Accounts (additional support coming soon)
...
- Add Guardian VS API as a provider option, allowing new users to sign up and get started with Guardian VS for free
...
- Improve OpenRouter/Guardian VS error reporting
...
- Add Plan/Act mode toggle to let you plan tasks with Guardian VS before letting him get to work
...
- Add ability to change viewport size and headless mode when Guardian VS asks to launch the browser
...
- Fix bug where Guardian VS couldn't read "@/" import path aliases from tool results
...
- Added checkpoints: Snapshots of workspace are automatically created whenever Guardian VS uses a tool
...
- Add support for Model Context Protocol (MCP), enabling Guardian VS to use custom tools like web-search tool or GitHub tool
- Add MCP server management tab accessible via the server icon in the menu bar
- Add ability for Guardian VS to dynamically create new MCP servers based on user requests (e.g., "add a tool that gets the latest npm docs")
...
- Add stricter prompt to prevent Guardian VS from editing files during a browser session without first closing the browser

## [2.1.0]

- Guardian VS now uses Anthropic's new "Computer Use" feature to launch a browser, click, type, and scroll. This gives him more autonomy in runtime debugging, end-to-end testing, and even general web use. Try asking "Look up the weather in Colorado" to see it in action! (Available with Claude 3.5 Sonnet v2)
...
- Fix bug where modifying Guardian VS's edits would lead him to try to re-apply the edits
...
- Update system prompt to try to prevent Guardian VS from lazy coding (`// rest of code here...`)
...
- Fixed bug where Guardian VS's edits would stream into the active tab when switching tabs during a write_to_file
...
- New name! Meet Guardian VS, an AI assistant that can use your CLI and Editor
- Responses are now streamed with a yellow text decoration animation to keep track of Guardian VS's progress as he edits files
- New Cancel button to give Guardian VS feedback if he goes off in the wrong direction, giving you more control over tasks
