...
- 195294f: Add support for bundled endpoints.json in enterprise distributions. Extensions can now include a pre-configured endpoints.json file that automatically switches Guardian to self-hosted mode. Includes packaging scripts for VSIX, NPM, and JetBrains plugins.
...
- 6d8fb85: Fix CLI crashing in CI environments and with stdin redirection (e.g., `guardian "prompt" < /dev/null`). Now checks both stdin and stdout TTY status before using Ink, and only errors on empty stdin when no prompt is provided.
