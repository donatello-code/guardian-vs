/**
 * GuardianAgent - Decoupled ACP Agent implementation for Guardian CLI.
 *
 * This class implements the ACP (Agent Client Protocol) Agent interface,
 * allowing Guardian to be used programmatically without stdio dependency.
...
import type { GuardianMessageChange } from "@core/task/message-state"
...
import type { GuardianAsk, GuardianMessage as GuardianMessageType } from "@shared/ExtensionMessage"
...
import { GuardianEndpoint } from "@/config.js"
...
import { GuardianSessionEmitter } from "./GuardianSessionEmitter.js"
import { translateMessage } from "./messageTranslator.js"
import { handlePermissionResponse } from "./permissionHandler.js"
import type { AcpSessionState, GuardianAcpSession, GuardianAgentOptions, PermissionHandler } from "./types.js"
...
/**
 * Guardian's implementation of the ACP Agent interface.
 *
 * This agent bridges the ACP protocol with Guardian's core Controller,
 ...
 * For stdio-based ACP communication, use the AcpAgent wrapper class.
 */
export class GuardianAgent implements acp.Agent {
	private readonly options: GuardianAgentOptions
	private readonly ctx: CliContextResult
	readonly sessions: Map<string, GuardianAcpSession> = new Map()
	...
	private readonly sessionEmitters: Map<string, GuardianSessionEmitter> = new Map()
	 ...
	 */
	emitterForSession(sessionId: string): GuardianSessionEmitter {
		let emitter = this.sessionEmitters.get(sessionId)
		if (!emitter) {
			emitter = new GuardianSessionEmitter()
	...
	async initialize(params: acp.InitializeRequest, connection?: acp.AgentSideConnection): Promise<acp.InitializeResponse> {
		...
		await GuardianEndpoint.initialize(this.ctx.EXTENSION_DIR)
		...
		return {
			...
			authMethods: [
				{
					id: "guardian-oauth",
					name: "Sign in with Guardian",
					description: "Authenticate with your Guardian account via browser OAuth",
				},
				{
	...
	async newSession(params: acp.NewSessionRequest): Promise<acp.NewSessionResponse> {
		...
		Logger.debug("[GuardianAgent] newSession called:", {
			sessionId,
			cwd: params.cwd,
		...
		const session: GuardianAcpSession = {
			sessionId,
			cwd: params.cwd,
		...
		await this.sendAvailableCommands(sessionId, controller).catch((error) => {
			Logger.debug("[GuardianAgent] Failed to send available commands:", error)
		})
	...
	private async getSessionModelState(mode: Mode): Promise<acp.SessionModelState> {
		...
		// Use provider-specific model ID key (e.g., guardian uses actModeOpenRouterModelId)
	...
	async unstable_setSessionModel(params: acp.SetSessionModelRequest): Promise<acp.SetSessionModelResponse> {
		...
		Logger.debug("[GuardianAgent] unstable_setSessionModel called:", {
		...
		// Update model ID using provider-specific keys (e.g., guardian uses actModeOpenRouterModelId)
	 ...
	 * 4. Translate GuardianMessages to ACP SessionUpdates
	...
	async prompt(params: acp.PromptRequest): Promise<acp.PromptResponse> {
		...
		Logger.debug("[GuardianAgent] prompt called:", {
			sessionId: params.sessionId,
		...
		try {
			...
			if (isLoadedSession && !hasActiveTask) {
				...
				Logger.debug("[GuardianAgent] Resuming loaded session:", params.sessionId)
				...
				Logger.debug("[GuardianAgent] Continuing existing task:", controller.task.taskId)

				// Find the last ask message and respond to it
				const messages = controller.task.messageStateHandler.getGuardianMessages()
				...
				if (lastAskMessage) {
					...
					Logger.debug("[GuardianAgent] No pending ask found, starting new task")
					await controller.initTask(textContent, imageContent, fileResources)
				}
			} else {
				// Start new task
				Logger.debug("[GuardianAgent] Starting new task")
				await controller.initTask(textContent, imageContent, fileResources)
			}

			// Subscribe to guardianMessages changes after task is created
			if (controller.task) {
				const onGuardianMessagesChanged = (change: GuardianMessageChange) => {
					this.handleGuardianMessagesChanged(params.sessionId, sessionState, change, resolvePrompt, promptResolved).catch(
						(error) => {
							Logger.debug("[GuardianAgent] Error handling guardianMessagesChanged:", error)
						},
					)
				}

				controller.task.messageStateHandler.on("guardianMessagesChanged", onGuardianMessagesChanged)
				cleanupFunctions.push(() => {
					controller.task?.messageStateHandler.off("guardianMessagesChanged", onGuardianMessagesChanged)
				})
			}
			...
			for (const cleanup of cleanupFunctions) {
				try {
					...
					Logger.debug("[GuardianAgent] Error during cleanup:", error)
				}
			}
	...
	private async handleGuardianMessagesChanged(
		sessionId: string,
		sessionState: AcpSessionState,
		change: GuardianMessageChange,
		resolvePrompt: (response: acp.PromptResponse) => void,
		promptResolved: { value: boolean },
	): Promise<void> {
		Logger.debug("[GuardianAgent] handleGuardianMessagesChanged:", change)
		try {
			switch (change.type) {
				case "add":
	 ...
	 * 3. Responds to Guardian's ask based on the decision
	 *
	 * @param sessionId - The session ID
	 * @param sessionState - The session state
	 * @param message - The Guardian ask message
	 * @param permissionRequest - The permission request details from translateMessage
	 */
	private async handlePermissionRequest(
		sessionId: string,
		sessionState: AcpSessionState,
		message: GuardianMessageType,
		...
		if (!controller?.task) {
			Logger.debug("[GuardianAgent] No active task for permission request")
			return
		}

		const askType = message.ask as GuardianAsk

		try {
			// Request permission from the client
			const response = await this.requestPermission(sessionId, permissionRequest.toolCall, permissionRequest.options)

			Logger.debug("[GuardianAgent] Permission response received:", response.outcome)
	 ...
	 *
	 * This method uses translateMessage to properly map GuardianMessages to ACP SessionUpdates,
	...
	private async processMessageWithDelta(
		...
		if (isTextStreamingMessage && message.text) {
			...
			if (result.requiresPermission && result.permissionRequest && !message.partial) {
				...
				// This will request permission from the client and respond to Guardian
	...
	async cancel(params: acp.CancelNotification): Promise<void> {
		...
		Logger.debug("[GuardianAgent] cancel called:", {
			sessionId: params.sessionId,
		...
		if (sessionState) {
			...
			if (controller?.task) {
				try {
					...
					Logger.debug("[GuardianAgent] Error cancelling task:", error)
				}
			}
		}
	}
	...
	async setSessionMode(params: acp.SetSessionModeRequest): Promise<acp.SetSessionModeResponse> {
		...
		Logger.debug("[GuardianAgent] setSessionMode called:", {
			sessionId: params.sessionId,
	 ...
	 * - Guardian account authentication (guardian-oauth)
	...
	async authenticate(params: acp.AuthenticateRequest): Promise<acp.AuthenticateResponse> {
		Logger.debug("[GuardianAgent] authenticate called:", { methodId: params.methodId })
		...
		if (params.methodId !== "guardian-oauth") {
		...
		Logger.debug("[GuardianAgent] AuthHandler enabled, getting callback URL...")

		// Get the callback URL first to ensure the server is ready
		let callbackUrl: string
		try {
			callbackUrl = await authHandler.getCallbackUrl("/auth")
			Logger.debug("[GuardianAgent] Callback URL ready:", callbackUrl)
		} catch (error) {
			Logger.error("[GuardianAgent] Failed to get callback URL:", error)
		...
		try {
			...
			Logger.debug("[GuardianAgent] Starting OAuth flow...")

			// Start the OAuth flow - this opens the browser
			await authService.createAuthRequest()

			Logger.debug("[GuardianAgent] Browser opened, waiting for callback...")
			...
			while (Date.now() - startTime < AUTH_TIMEOUT_MS) {
				...
				const authData = stateManager.getSecretKey("guardian:guardianAccountId")
				if (authData) {
					Logger.debug("[GuardianAgent] Authentication successful")

					// Set up the provider configuration for guardian
					stateManager.setGlobalState("actModeApiProvider", "guardian")
					stateManager.setGlobalState("planModeApiProvider", "guardian")
					await stateManager.flushPendingState()
			...
			Logger.error("[GuardianAgent] Authentication error:", error)
			throw error
		}
	}
	...
	private async emitSessionUpdate(sessionId: string, update: acp.SessionUpdate): Promise<void> {
		...
		try {
			...
			Logger.debug("[GuardianAgent] Error emitting session update:", error)
	...
	protected async requestPermission(
		...
		Logger.debug("[GuardianAgent] Requesting permission:", {
		...
		if (!this.permissionHandler) {
			...
			Logger.debug("[GuardianAgent] No permission handler set, auto-rejecting")
	 ...
	 *
	 * This fetches commands from Guardian's slash command system, filters out
	...
	private async sendAvailableCommands(sessionId: string, controller: Controller): Promise<void> {
		try {
			// Get all available commands from Guardian
			...
			Logger.debug("[GuardianAgent] Sent available commands:", {
				sessionId,
				commandCount: availableCommands.length,
				commands: availableCommands.map((c) => c.name),
			})
		} catch (error) {
			Logger.debug("[GuardianAgent] Error sending available commands:", error)
		}
	}
	 ...
	 * - Guardian provider with stored auth data
	...
	private async isAuthConfigured(): Promise<boolean> {
		...
		const providerKey = mode === "act" ? "actModeApiProvider" : "planModeApiProvider"
		const currentProvider = (stateManager.getGlobalSettingsKey(providerKey) as string) || "guardian"

		if (currentProvider === "guardian") {
			// For Guardian provider, check if we have stored auth data
			return Boolean(stateManager.getSecretKey("guardianApiKey") || stateManager.getSecretKey("guardianAccountId"))
		}
	...
	private async authenticateOpenAiCodex(): Promise<acp.AuthenticateResponse> {
		Logger.debug("[GuardianAgent] Starting OpenAI Codex OAuth flow...")

		try {
			// Get the authorization URL and start the callback server
			const authUrl = openAiCodexOAuthManager.startAuthorizationFlow()

			Logger.debug("[GuardianAgent] Opening browser for OpenAI Codex auth:", authUrl)
			...
			Logger.debug("[GuardianAgent] OpenAI Codex authentication successful")
			...
			Logger.error("[GuardianAgent] OpenAI Codex authentication error:", error)
			throw error
		}
	}
}
