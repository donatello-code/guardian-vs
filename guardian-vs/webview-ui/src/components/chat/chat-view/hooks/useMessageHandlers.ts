...
export function useMessageHandlers(messages: ClineMessage[], chatState: ChatState): MessageHandlers {
	...
	// Handle adding a prompt to the queue
	const handleAddToQueue = useCallback(
		async (text: string, images: string[], files: string[]) => {
			...
			if (hasContent) {
				console.log("[ChatView] handleAddToQueue - Adding to queue:", messageToSend)
				
				await StateServiceClient.addToQueue(
					AddToQueueRequest.create({
		...
		[activeQuote, setInputValue, setActiveQuote, setSelectedImages, setSelectedFiles],
	)
