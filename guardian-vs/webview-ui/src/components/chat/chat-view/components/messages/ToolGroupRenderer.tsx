import { GuardianMessage, GuardianSayTool } from "@shared/ExtensionMessage"
...
interface ToolWithReasoning {
	tool: GuardianMessage
	parsedTool: GuardianSayTool
	reasoning?: string
	isActive?: boolean
	activityText?: string
}
...
const getCurrentActivities = (allMessages: GuardianMessage[]): GuardianMessage[] => {
	...
	const activities: GuardianMessage[] = []
 ...
 */
function buildToolsWithReasoning(messages: GuardianMessage[]): ToolWithReasoning[] {
 ...
 */
function parseToolSafe(text: string | undefined): GuardianSayTool {
	try {
		return JSON.parse(text || "{}") as GuardianSayTool
	} catch {
		return {} as GuardianSayTool
	}
}
 ...
 */
function getToolDisplayInfo(tool: GuardianSayTool) {
 ...
 */
function getToolGroupSummary(messages: GuardianMessage[]): string {
	...
	return parts.length === 0 ? "Context" : "Guardian" + action + parts.join(", ")
}
