...
describe("getButtonConfig", () => {
	...
	it("returns partial config for streaming messages", () => {
		const streamingMessage: GuardianMessage = {
			type: "say",
			say: "api_req_started",
			partial: true,
			ts: Date.now(),
	...
	describe("Tool Approval States", () => {
		it("returns tool_approve config for generic tool ask", () => {
			const toolMessage: GuardianMessage = {
				type: "ask",
				ask: "tool",
	...
	describe("Command Execution States", () => {
		it("returns command config for command ask", () => {
			const commandMessage: GuardianMessage = {
				type: "ask",
				ask: "command",
				ts: Date.now(),
			}
		...
		it("returns command_output config for command_output ask", () => {
			const commandOutputMessage: GuardianMessage = {
				type: "ask",
				ask: "command_output",
				ts: Date.now(),
			}
	...
	it("returns api_req_active config for api_req_started say message", () => {
		const apiReqMessage: GuardianMessage = {
			type: "say",
			say: "api_req_started",
			ts: Date.now(),
		}
