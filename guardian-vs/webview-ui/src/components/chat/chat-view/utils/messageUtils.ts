 ...
 */
export function isLowStakesTool(message: GuardianMessage): boolean {
 ...
 */
export function isToolGroup(item: GuardianMessage | GuardianMessage[]): item is GuardianMessage[] & { _isToolGroup: true } {
 ...
 */
export function processMessages(messages: GuardianMessage[]): GuardianMessage[] {
 ...
 */
export function filterVisibleMessages(messages: GuardianMessage[]): GuardianMessage[] {
	return messages.filter((message, index, arr) => {
		switch (message.ask) {
			case "completion_result":
				// don't show a chat row for a completion_result ask without text. This specific type of message only occurs if guardian wants to execute a command as part of its completion result, in which case we interject the completion_result tool with the execute_command tool.
				if (message.text === "") {
					return false
				}
				break
		...
		switch (message.say) {
			...
			case "text":
				// Sometimes guardian returns an empty text message, we don't want to render these. (We also use a say text for user messages, so in case they just sent images we still render that)
 ...
 */
export function isBrowserSessionMessage(message: GuardianMessage): boolean {
	if (message.type === "ask") {
 ...
 */
export function groupMessages(visibleMessages: GuardianMessage[]): (GuardianMessage | GuardianMessage[])[] {
	const result: (GuardianMessage | GuardianMessage[])[] = []
	let currentGroup: GuardianMessage[] = []
		...
		if (message.ask === "browser_action_launch" || message.say === "browser_action_launch") {
			...
			if (isBrowserSessionMessage(message)) {
				...
				if (message.say === "browser_action") {
					const browserAction = JSON.parse(message.text || "{}") as GuardianSayBrowserAction
 ...
 */
export function getTaskMessage(messages: GuardianMessage[]): GuardianMessage | undefined {
 ...
 */
function isDisplayedCheckpoint(checkpointIndex: number, allMessages: GuardianMessage[]): boolean {
	...
	for (let i = checkpointIndex - 1; i >= 0; i--) {
		...
		if (msg.say === "tool" || msg.ask === "tool") {
			try {
				const tool = JSON.parse(msg.text || "{}") as GuardianSayTool
 ...
 */
export function findNextSegmentCost(checkpointTs: number, allMessages: GuardianMessage[]): number | undefined {
	const checkpointIndex = allMessages.findIndex((m) => m.ts === checkpointTs && m.say === "checkpoint_created")
	if (checkpointIndex === -1) {
		return undefined
	}
 ...
 */
export function isTextMessagePendingToolCall(textTs: number, allMessages: GuardianMessage[]): boolean {
 ...
 */
export function isToolGroupInFlight(toolGroupMessages: GuardianMessage[], allMessages: GuardianMessage[]): boolean {
	if (toolGroupMessages.length === 0) {
		return false
	}

	// Step 1: Find the MOST RECENT api_req_started overall (search backwards)
	let mostRecentApiReq: GuardianMessage | null = null
 ...
 */
export function getToolsNotInCurrentActivities(toolGroupMessages: GuardianMessage[], allMessages: GuardianMessage[]): GuardianMessage[] {
 ...
 *
 * Note: this operates on a flat `GuardianMessage[]` (e.g. `modifiedMessages`) rather than
 ...
 */
export function isApiReqAbsorbable(apiReqTs: number, allMessages: GuardianMessage[]): boolean {
	const apiReqIndex = allMessages.findIndex((m) => m.ts === apiReqTs && m.say === "api_req_started")
	if (apiReqIndex === -1) {
		return false
	}
 ...
 */
function isApiReqFollowedOnlyByLowStakesTools(index: number, messages: (GuardianMessage | GuardianMessage[])[]): boolean {
	let hasLowStakesTool = false
	let hasReasoning = false
 ...
 */
export function groupLowStakesTools(groupedMessages: (GuardianMessage | GuardianMessage[])[]): (GuardianMessage | GuardianMessage[])[] {
	const result: (GuardianMessage | GuardianMessage[])[] = []
	let toolGroup: GuardianMessage[] = []
	let pendingReasoning: GuardianMessage[] = []
	let pendingApiReq: GuardianMessage[] = []
	let hasTools = false
	const pendingTools: GuardianMessage[] = []
	...
	const commitToolGroup = () => {
		if (toolGroup.length > 0 && hasTools) {
			const group = toolGroup as GuardianMessage[] & { _isToolGroup: boolean }
