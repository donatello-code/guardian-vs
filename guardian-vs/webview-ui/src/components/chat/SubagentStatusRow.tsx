...
function parseSubagentRowData(message: GuardianMessage): SubagentRowData | null {
	if (!message.text) {
		return null
	}
	...
	try {
		if (message.ask === "use_subagents" || message.say === "use_subagents") {
			const parsed = JSON.parse(message.text) as GuardianAskUseSubagents
		...
		const parsed = JSON.parse(message.text) as GuardianSaySubagentStatus
...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	const title = singular ? "Guardian wants to use a subagent:" : "Guardian wants to use subagents:"
