...
import { BrowserAction, BrowserActionResult, GuardianMessage, GuardianSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: GuardianMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: GuardianMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: GuardianMessage[] = []
		let nextActionMessages: GuardianMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? "Guardian is using the browser:" : "Guardian wants to use the browser:"}
				</span>
			</div>
			<div
				style={{
