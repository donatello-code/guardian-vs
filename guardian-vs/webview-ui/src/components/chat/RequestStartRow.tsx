...
const getActivityText = (tool: GuardianSayTool): string | null => {
...
const collectToolsInRange = (
	...
	startIdx: number,
	endIdx: number,
	stopCondition?: (msg: GuardianMessage) => boolean,
...
const findCurrentApiReq = (messages: GuardianMessage[]): { index: number; hasCost: boolean } | null => {
...
const findPrevCompletedApiReq = (messages: GuardianMessage[], beforeIdx: number): number => {
...
export const RequestStartRow: React.FC<RequestStartRowProps> = ({
	...
	const hasCompletionResult = guardianMessages.some(
	...
	const willBeAbsorbed = useMemo(() => {
		return isApiReqAbsorbable(message.ts, guardianMessages)
	}, [message.ts, guardianMessages])
	...
	const currentActivities = useMemo(() => {
		const currentApiReq = findCurrentApiReq(guardianMessages)
		if (!currentApiReq) {
			return []
		}
		...
		if (!currentApiReq.hasCost) {
			...
			return collectToolsInRange(guardianMessages, currentApiReq.index + 1, guardianMessages.length)
		}
	...
	const hasCompletedTools = useMemo(() => {
		...
		return guardianMessages.some((msg, idx) => {
			if (msg.say === "tool" && isLowStakesTool(msg)) {
				...
				for (let i = idx - 1; i >= 0; i--) {
					const prevMsg = guardianMessages[i]
