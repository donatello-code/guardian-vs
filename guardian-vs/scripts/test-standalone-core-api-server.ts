...
/**
 * Simple Guardian gRPC Server
 *
 * This script provides a minimal way to run the Guardian core gRPC service
 ...
 *   2. GuardianApiServerMock (mock implementation of the Guardian API)
 ...
 *   CLINE_CORE_FILE - Override core file name (default: guardian-core.js)
...
import { GuardianApiServerMock } from "../src/test/e2e/fixtures/server/index"
...
const guardianCoreFile = process.env.CLINE_CORE_FILE || "guardian-core.js"
const coreFile = path.join(distDir, guardianCoreFile)

const childProcesses: ChildProcess[] = []

async function main(): Promise<void> {
	console.log("Starting Simple Guardian gRPC Server...")
	...
	try {
		...
		console.log("Guardian API Server started in-process")
	} catch (error) {
		console.error("Failed to start Guardian API Server:", error)
		process.exit(1)
	}

	const extensionsDir = path.join(distDir, "vsce-extension")
	const userDataDir = mkdtempSync(path.join(os.tmpdir(), "vsce"))
	const guardianTestWorkspace = mkdtempSync(path.join(os.tmpdir(), "guardian-test-workspace-"))
	...
	const hostbridge: ChildProcess = spawn("npx", ["tsx", path.join(__dirname, "test-hostbridge-server.ts")], {
		...
		env: {
			...process.env,
			TEST_HOSTBRIDGE_WORKSPACE_DIR: guardianTestWorkspace,
	...
	const baseArgs = ["--enable-source-maps", path.join(distDir, "guardian-core.js")]

	const spawnArgs = USE_C8 ? ["c8", "--report-dir", covDir, "node", ...baseArgs] : ["node", ...baseArgs]

	console.log(`Starting Guardian Core Service... (useC8=${USE_C8})`)
	...
	const shutdown = async () => {
		...
		await GuardianApiServerMock.stopGlobalServer()

		try {
			rmSync(userDataDir, { recursive: true, force: true })
			rmSync(guardianTestWorkspace, { recursive: true, force: true })
	...
	console.log(`Guardian gRPC Server is running on 127.0.0.1:${PROTOBUS_PORT}`)
	console.log("Press Ctrl+C to stop")
}

if (require.main === module) {
	main().catch((err) => {
		console.error("Failed to start simple Guardian server:", err)
		process.exit(1)
	})
}
