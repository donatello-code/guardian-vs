...
async function main() {
	log("\n\n\nStarting guardian-core service...\n\n\n")
...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// guardian-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the guardian-core process by:
...
function showHelp() {
	Logger.log(`
Guardian Core - Standalone Server

Usage: node guardian-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for Guardian data storage (default: ~/.guardian)
