...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "guardian-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .guardianrules/workflows directory specifically", async () => {
		// Create a test directory structure
		const guardianrulesDirTest = path.join(tmpDir, "guardianrules-test")
		const guardianrulesDirPath = path.join(guardianrulesDirTest, ".guardianrules")

		// Create .guardianrules directory and root files
		await fs.mkdir(guardianrulesDirPath, { recursive: true })
		await fs.writeFile(path.join(guardianrulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(guardianrulesDirPath, "settings.js"), "// settings")

		// Create .guardianrules/other directory and files
		const otherDirPath = path.join(guardianrulesDirPath, "other")
		...
		// Create .guardianrules/workflows directory and files
		const workflowsDirPath = path.join(guardianrulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(guardianrulesDirPath)
		...
		const filteredFiles = await readDirectory(guardianrulesDirPath, [[".guardianrules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(guardianrulesDirPath, "config.json"),
			path.resolve(guardianrulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(guardianrulesDirPath, [
		...
		const rootOnlyFiles = [path.resolve(guardianrulesDirPath, "config.json"), path.resolve(guardianrulesDirPath, "settings.js")]

		multiExcludeFiles.sort().should.deepEqual(rootOnlyFiles.sort())
	})

	it("should exclude .guardianrules/hooks directory specifically", async () => {
		// Create a test directory structure
		const guardianrulesDirTest = path.join(tmpDir, "guardianrules-hooks-test")
		const guardianrulesDirPath = path.join(guardianrulesDirTest, ".guardianrules")
		...
		// Create .guardianrules/hooks directory and files
		const hooksDirPath = path.join(guardianrulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(guardianrulesDirPath, [[".guardianrules", "hooks"]])
