...
const prefix = name === "claude-dev" ? "guardian" : name
 ...
 * In production, all commands are registered under "guardian" for consistency.
 */
const GuardianCommands = {
	...
	FixWithGuardian: prefix + ".fixWithGuardian",
...
export interface HostInfo {
	...
	os: string
	/**
	 * The type of the guardian host environment, e.g. 'VSCode Extension', 'Guardian for JetBrains', 'CLI'
	...
	hostVersion?: string
	/**
	 * The version of Guardian that the host client is running
	 */
...
export const HostRegistryInfo = {
	init: async (distinctId: string) => {
		...
		const extensionVersion = host.guardianVersion || ExtensionRegistryInfo.version
		const platform = host.platform || "unknown"
		const os = process.platform || "unknown"
		const ide = host.guardianType || "unknown"
