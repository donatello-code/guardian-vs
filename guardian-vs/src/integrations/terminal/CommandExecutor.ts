...
import { GuardianToolResponseContent } from "@shared/messages"
...
export class CommandExecutor {
	...
	async cancelBackgroundCommand(): Promise<boolean> {
		...
		if (cancelled) {
			...
			const messages = this.callbacks.getGuardianMessages()
			...
			if (lastCommandOutputIndex !== -1) {
				...
				await this.callbacks.updateGuardianMessage(lastCommandOutputIndex, {
