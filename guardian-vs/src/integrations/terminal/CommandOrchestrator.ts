...
export async function orchestrateCommandExecution(
	...
	const clearCommandState = async () => {
		...
		const guardianMessages = callbacks.getGuardianMessages()
		const lastCommandIndex = findLastIndex(guardianMessages, (m) => m.ask === "command" || m.say === "command")
		if (lastCommandIndex !== -1) {
			await callbacks.updateGuardianMessage(lastCommandIndex, {
	...
	const switchToFileBased = async () => {
		...
		// Set up file logging using GuardianTempManager for proper cleanup
		largeOutputLogPath = GuardianTempManager.createTempFilePath("large-output")
