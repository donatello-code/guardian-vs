		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to Guardian VS (Always available)
					const addAction = new vscode.CodeAction("Add to Guardian VS", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: commands.AddToChat,
						title: "Add to Guardian VS",
						arguments: [expandedRange, context.diagnostics],
					}
					actions.push(addAction)

					// Explain with Guardian VS (Always available)
					const explainAction = new vscode.CodeAction("Explain with Guardian VS", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: commands.ExplainCode,
						title: "Explain with Guardian VS",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with Guardian VS (Always available)
					const improveAction = new vscode.CodeAction("Improve with Guardian VS", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: commands.ImproveCode,
						title: "Improve with Guardian VS",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with Guardian VS (Only if diagnostics exist)
...
\`\`\``
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.Walkthrough, async () => {
			await vscode.commands.executeCommand("workbench.action.openWalkthrough", `${context.extension.id}#GuardianVSWalkthrough`)
