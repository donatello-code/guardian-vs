...
This directory contains the end-to-end tests for the Guardian VS Code extension using Playwright. These tests simulate user interactions with the extension in a real VS Code environment.
...
- **`fixtures/server/`** - Mock API server for testing Guardian's backend interactions
...
e2e("Test description", async ({ sidebar, helper, page }) => {
  ...
  await inputbox.fill("Hello, Guardian!")
...
- **`sidebar`** - Playwright Frame object for the Guardian extension's sidebar
...
await addSelectedCodeToGuardianWebview(page)
```
...
2. **Playwright will open:**
   - A VS Code window with Guardian extension loaded
...
- **VS Code Configuration:**
  ...
  - Extension development mode with Guardian loaded
  - Temporary user data and extensions directories

- **Mock API Server:**
  - Runs on `http://localhost:7777`
  - Provides mock responses for Guardian API calls
