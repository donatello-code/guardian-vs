...
import type { BalanceResponse, OrganizationBalanceResponse, UserResponse } from "../../../../shared/GuardianAccount"
import { E2E_MOCK_API_RESPONSES, E2E_REGISTERED_MOCK_ENDPOINTS } from "./api"
import { GuardianDataMock } from "./data"
...
function log(...args: unknown[]) {
	if (useVerboseLogging) {
		console.log("[GuardianApiServerMock]", ...args)
	}
}

export class GuardianApiServerMock {
	static globalSharedServer: GuardianApiServerMock | null = null
	...
	public readonly API_USER = new GuardianDataMock("personal")
	...
	public static async startGlobalServer(): Promise<GuardianApiServerMock> {
		log("=== SERVER FIXTURE CALLED ===")
		if (GuardianApiServerMock.globalSharedServer) {
			log("Using existing global server")
			return GuardianApiServerMock.globalSharedServer
		}
		...
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			...
			if (isAuthRequired && authToken) {
				...
				const user = GuardianApiServerMock.globalSharedServer!.API_USER.getUserByToken(authToken)
				if (!user) {
					return sendApiError("Invalid token", 401)
				}
				GuardianApiServerMock.globalSharedServer!.setCurrentUser(user)
			}
			...
			const handleRequest = async () => {
				...
				const routeMatch = GuardianApiServerMock.matchRoute(path, method)
				...
				const controller = GuardianApiServerMock.globalSharedServer!
				...
				if (baseRoute === "/api/v1") {
					...
					if (endpoint === "/auth/token" && method === "POST") {
						...
						// Return format matching GuardianAuthProvider expectations
						return sendApiResponse({
					...
					if (endpoint === "/chat/completions" && method === "POST") {
						...
						if (body.includes("[diff.test.ts] Hello, Guardian!")) {
								...
								choices: [
									{
										...
										message: {
											role: "assistant",
											content: "Hello! I'm a mock Guardian API response.",
										},
										finish_reason: "stop",
									},
								],
		...
		const controller = new GuardianApiServerMock(server)
		GuardianApiServerMock.globalSharedServer = controller

		// Track connections for proper cleanup
		server.on("connection", (socket) => {
			GuardianApiServerMock.globalSockets.add(socket)
			socket.on("close", () => {
				GuardianApiServerMock.globalSockets.delete(socket)
			})
		})
		...
		await new Promise<void>((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error?: Error) => {
				if (error) {
					...
					log(`GuardianApiServerMock listening on port ${E2E_API_SERVER_PORT}`)
					resolve()
				}
			})
		})
	...
	public static async stopGlobalServer(): Promise<void> {
		...
		const server = GuardianApiServerMock.globalSharedServer.server

		// Clean shutdown - destroy all socket connections first
		GuardianApiServerMock.globalSockets.forEach((socket) => socket.destroy())
		GuardianApiServerMock.globalSockets.clear()
		...
		GuardianApiServerMock.globalSharedServer = null
	}
}
