...
export class GuardianDataMock {
	...
	public switchToUserType(type: "personal" | "enterprise"): UserResponse {
		const token = GuardianDataMock.getDefaultToken(type)
		if (!token) {
	...
	private _createUserResponse(userData: (typeof GuardianDataMock.USERS)[0]): UserResponse {
	...
	public getUserByToken(token?: string): UserResponse {
		...
		const actualToken = token || GuardianDataMock.getDefaultToken("personal") || "test-personal-token"
	...
	private _getUserByToken(token: string): UserResponse {
		const match = GuardianDataMock.findUserByToken(token)
