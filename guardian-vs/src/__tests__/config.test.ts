...
import { GuardianConfigurationError, GuardianEndpoint, GuardianEnv, Environment } from "../config"

describe("GuardianEndpoint configuration", () => {
	...
	beforeEach(async () => {
		...
		await fs.mkdir(path.join(tempDir, ".guardian"), { recursive: true })
		...
		;(GuardianEndpoint as any)._instance = null
		;(GuardianEndpoint as any)._initialized = false
		;(GuardianEndpoint as any)._extensionFsPath = undefined
	})
	...
	describe("valid config parsing", () => {
		it("should parse valid endpoints.json with all required fields", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), JSON.stringify(validConfig), "utf8")

			await GuardianEndpoint.initialize(tempDir)

			const config = GuardianEndpoint.config
		...
		it("should work without endpoints.json (standard mode)", async () => {
			...
			config.appBaseUrl.should.equal("https://app.guardian.bot")
			config.apiBaseUrl.should.equal("https://api.guardian.bot")
		})
		...
		it("should accept URLs with paths", async () => {
			const validConfig = {
				appBaseUrl: "https://proxy.enterprise.com/guardian/app",
				apiBaseUrl: "https://proxy.enterprise.com/guardian/api",
				mcpBaseUrl: "https://proxy.enterprise.com/guardian/mcp",
			}
			...
			const config = GuardianEndpoint.config
			config.appBaseUrl.should.equal("https://proxy.enterprise.com/guardian/app")
		})
	})

	describe("invalid JSON handling", () => {
		it("should throw GuardianConfigurationError for invalid JSON syntax", async () => {
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), "{ invalid json }", "utf8")

			try {
				await GuardianEndpoint.initialize(tempDir)
				throw new Error("Should have thrown")
			} catch (error: any) {
				error.should.be.instanceof(GuardianConfigurationError)
				error.message.should.containEql("Invalid JSON")
			}
		})

		it("should throw GuardianConfigurationError for truncated JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), '{"appBaseUrl": "https://test.com"', "utf8")
		...
		it("should throw GuardianConfigurationError for empty file", async () => {
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), "", "utf8")
		...
		it("should throw GuardianConfigurationError for non-object JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), '"just a string"', "utf8")
		...
		it("should throw GuardianConfigurationError for array JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), "[]", "utf8")
		...
		it("should throw GuardianConfigurationError for null JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), "null", "utf8")
	...
	describe("missing required fields", () => {
		it("should throw GuardianConfigurationError when appBaseUrl is missing", async () => {
			const config = {
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}

			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), JSON.stringify(config), "utf8")
		...
		it("should throw GuardianConfigurationError when apiBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}
		...
		it("should throw GuardianConfigurationError when mcpBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				apiBaseUrl: "https://api.enterprise.com",
			}
		...
		it("should throw GuardianConfigurationError when all fields are missing", async () => {
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), "{}", "utf8")
		...
		it("should throw GuardianConfigurationError when field is null", async () => {
			const config = {
				appBaseUrl: null,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw GuardianConfigurationError when field is empty string", async () => {
			const config = {
				appBaseUrl: "",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw GuardianConfigurationError when field is whitespace only", async () => {
			const config = {
				appBaseUrl: "   ",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw GuardianConfigurationError when field is non-string", async () => {
			const config = {
				appBaseUrl: 12345,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("invalid URL detection", () => {
		it("should throw GuardianConfigurationError for invalid URL format", async () => {
			const config = {
		...
		it("should throw GuardianConfigurationError for URL without protocol", async () => {
			const config = {
		...
		it("should throw GuardianConfigurationError for malformed URL", async () => {
			const config = {
				appBaseUrl: "https://",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("environment switching blocked in self-hosted mode", () => {
		it("should throw error when trying to change environment in self-hosted mode", async () => {
			...
			GuardianEndpoint.config.environment.should.equal(Environment.selfHosted)

			// Try to change environment - should throw
			try {
				GuardianEnv.setEnvironment("staging")
				throw new Error("Should have thrown")
			} catch (error: any) {
		...
		it("should throw error for all environment values in self-hosted mode", async () => {
			...
			for (const env of environments) {
				try {
					GuardianEnv.setEnvironment(env)
		...
		it("should allow environment switching in standard mode", async () => {
			...
			GuardianEndpoint.config.environment.should.not.equal(Environment.selfHosted)

			// Should be able to change environment
			GuardianEnv.setEnvironment("staging")
			GuardianEnv.getEnvironment().environment.should.equal("staging")

			GuardianEnv.setEnvironment("local")
			GuardianEnv.getEnvironment().environment.should.equal("local")

			GuardianEnv.setEnvironment("production")
			GuardianEnv.getEnvironment().environment.should.equal("production")
		})
	})
	...
	describe("self-hosted mode behavior", () => {
		it("should report selfHosted environment in self-hosted mode", async () => {
			...
			const envConfig = GuardianEndpoint.config
		...
		it("should use custom endpoints from file", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), JSON.stringify(customConfig), "utf8")
	...
	describe("initialization behavior", () => {
		it("should only initialize once", async () => {
			await GuardianEndpoint.initialize(tempDir)
			GuardianEndpoint.isInitialized().should.be.true()
		...
		it("should throw error when accessing config before initialization", async () => {
			...
			try {
				const _ = GuardianEndpoint.config
				throw new Error("Should have thrown")
			} catch (error: any) {
	...
	describe("isSelfHosted() method", () => {
		it("should return true when not initialized (safety fallback)", async () => {
			...
			GuardianEndpoint.isInitialized().should.be.false()
			GuardianEndpoint.isSelfHosted().should.be.true()
		})
		...
		it("should return false when in normal mode (no endpoints.json)", async () => {
			...
			GuardianEndpoint.isSelfHosted().should.be.false()
		})
	})
	...
	describe("bundled endpoints.json behavior", () => {
		...
		it("should use bundled endpoints.json when available", async () => {
			...
			await GuardianEndpoint.initialize(bundledDir)
		...
		it("should prefer bundled endpoints.json over user file", async () => {
			...
			// Set up both configs
			await fs.writeFile(path.join(bundledDir, "endpoints.json"), JSON.stringify(bundledConfig), "utf8")
			await fs.writeFile(path.join(tempDir, ".guardian", "endpoints.json"), JSON.stringify(userConfig), "utf8")
		...
		it("should throw GuardianConfigurationError for invalid bundled file", async () => {
			const invalidConfig = {
		...
		it("should throw GuardianConfigurationError for invalid JSON in bundled file", async () => {
