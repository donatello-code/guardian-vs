...
describe("Remote Config Schema", () => {
	...
	describe("GuardianSettingsSchema", () => {
		it("should accept valid Guardian provider settings", () => {
			const validSettings = {
				models: [{ id: "claude-3-5-sonnet-20241022" }, { id: "claude-3-5-haiku-20241022" }],
			}
			const result = GuardianSettingsSchema.parse(validSettings)
			expect(result).to.deep.equal(validSettings)
		})

		it("should accept empty settings object", () => {
			const result = GuardianSettingsSchema.parse({})
			expect(result.models).to.be.undefined
		})
		...
		it("should accept models with only id field", () => {
			...
			expect(() => GuardianSettingsSchema.parse(settings)).to.not.throw()
		})

		it("should reject models with missing id field", () => {
			expect(() =>
				GuardianSettingsSchema.parse({
					models: [{}],
				}),
			).to.throw()
		})
	...
	describe("RemoteConfigSchema", () => {
		...
		it("should handle complete config with all fields", () => {
			const config = {
				...
				providerSettings: {
					...
					Anthropic: {
						...
						baseUrl: "https://example.guardian.bot",
					},
				},
				enterpriseTelemetry: {
					promptUploading: {
			...
			// Verify Guardian settings
			expect(result.providerSettings?.Guardian?.models).to.have.lengthOf(2)
			expect(result.providerSettings?.Guardian?.models?.[0].id).to.equal("claude-3-5-sonnet-20241022")
			expect(result.providerSettings?.Guardian?.models?.[1].id).to.equal("claude-3-5-haiku-20241022")
			...
			expect(result.providerSettings?.Anthropic?.baseUrl).to.equal("https://example.guardian.bot")
