...
export interface GuardianStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: GuardianStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and GuardianSyncStorage both satisfy this interface.
 */
export interface GuardianMemento {
 ...
 */
export abstract class GuardianStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "GuardianStorage"
	/**
 ...
 * Unlike GuardianStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: GuardianStorageChangeEvent) => void

export abstract class GuardianSyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of GuardianStorage using a Map.
 */
export class GuardianInMemoryStorage extends GuardianStorage {
	/**
