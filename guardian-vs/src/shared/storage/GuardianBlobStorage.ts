...
import { GuardianStorage } from "./GuardianStorage"
...
/**
 * S3/R2 blob storage implementation of GuardianStorage.
 * Uses AWS S3 or Cloudflare R2 as the backend storage.
 */
export class GuardianBlobStorage extends GuardianStorage {
	override name = "GuardianBlobStorage"

	private static store: GuardianBlobStorage | null = null
	static get instance(): GuardianBlobStorage {
		if (!GuardianBlobStorage.store) {
			GuardianBlobStorage.store = new GuardianBlobStorage()
		}
		return GuardianBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!GuardianBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[GuardianBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[GuardianBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = GuardianBlobStorage.instance
