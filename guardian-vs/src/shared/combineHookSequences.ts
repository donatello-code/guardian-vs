import { GuardianMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the GuardianSayHook interface but represents parsed data.
 */
...
function getSay(msg: GuardianMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: GuardianMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: GuardianMessage): HookMetadata | null {
 ...
 */
function dedupeToolOrCommandMessagesByTimestamp(messages: GuardianMessage[]): GuardianMessage[] {
...
function combineHookWithOutputs(
	hookMessage: GuardianMessage,
	startIndex: number,
	messages: GuardianMessage[],
): { combined: GuardianMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: GuardianMessage[]): GuardianMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, GuardianMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: GuardianMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: GuardianMessage[], originalMessages: GuardianMessage[]): Map<number, GuardianMessage[]> {
	const map = new Map<number, GuardianMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: GuardianMessage[], preToolUseMap: Map<number, GuardianMessage[]>): GuardianMessage[] {
	const result: GuardianMessage[] = []
 ...
 *
 * @param messages Array of GuardianMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: GuardianMessage[]): GuardianMessage[] {
