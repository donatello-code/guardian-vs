...
export function registerTaskCommands(controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("guardian.dev.expireMcpOAuthTokens", async () => {
			try {
		...
		vscode.commands.registerCommand("guardian.dev.createTestTasks", async () => {
			const count = (
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): GuardianMessage[] {
	...
	const messages: GuardianMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in Guardian.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
