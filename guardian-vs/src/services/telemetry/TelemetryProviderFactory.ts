...
export type TelemetryProviderConfig =
	 ...
	 * @param bypassUserSettings - When true, telemetry is sent regardless of the user's Guardian telemetry opt-in/opt-out settings.
	 * This is used for:
...
export class TelemetryProviderFactory {
	...
	public static getDefaultConfigs(): TelemetryProviderConfig[] {
		...
		if (!GuardianEndpoint.isSelfHosted() && isPostHogConfigValid(posthogConfig)) {
			configs.push({ type: "posthog", ...posthogConfig })
		}

		// Skip build-time OTEL in selfHosted mode - enterprise customers should not send telemetry to Guardian's collector
		...
		if (!GuardianEndpoint.isSelfHosted() && otelConfig) {
			configs.push({
				type: "opentelemetry",
				config: otelConfig,
		...
		if (runtimeOtelConfig) {
			configs.push({
				...
				// OTEL regardless of his Guardian telemetry settings
