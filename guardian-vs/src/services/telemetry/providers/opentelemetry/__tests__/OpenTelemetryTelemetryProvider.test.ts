...
import type { GuardianAccountUserInfo } from "@/services/auth/AuthService"
...
function makeUserInfo(
	overrides: Partial<GuardianAccountUserInfo> & { orgOverrides?: Record<string, unknown> } = {},
): GuardianAccountUserInfo {
...
describe("OpenTelemetryTelemetryProvider.identifyUser", () => {
	...
	it("should handle user with no active organization", () => {
		...
		const userInfo: GuardianAccountUserInfo = {
			id: "user-1",
	...
	it("should clear stale org attributes when switching from org to no-org", () => {
		...
		const userNoOrg: GuardianAccountUserInfo = {
			id: "user-1",
			displayName: "Test User",
			email: "test@example.com",
			createdAt: new Date().toISOString(),
