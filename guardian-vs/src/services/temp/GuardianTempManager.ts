/**
 * GuardianTempManager - Manages temporary files for Guardian with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "guardian" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for Guardian's temporary files.
 */
class GuardianTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "guardian" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/guardian
		// Windows: C:\Users\{user}\AppData\Local\Temp\guardian
		// Linux: /tmp/guardian
		const baseTempDir = os.tmpdir()
		const guardianTempDir = path.join(baseTempDir, "guardian")

		try {
			fs.mkdirSync(guardianTempDir, { recursive: true })
			this.tempDir = guardianTempDir
		} catch {
	...
	/**
	 * Clean up old Guardian temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Guardian temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`Guardian temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during Guardian temp cleanup", error)
		}
...
export const GuardianTempManager = new GuardianTempManagerImpl()
