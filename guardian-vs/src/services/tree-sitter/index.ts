...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = guardianIgnoreController ? guardianIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, guardianIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (guardianIgnoreController && !guardianIgnoreController.validateAccess(filePath)) {
		return null
	}
