...
import type { BannerRules } from "@shared/GuardianBanner"
import { expect } from "chai"
import { afterEach, beforeEach, describe, it } from "mocha"
import * as sinon from "sinon"
import { GuardianEnv, Environment } from "@/config"
...
describe("BannerService", () => {
	...
	beforeEach(() => {
		...
		// Mock GuardianEnv.config() to return a valid config
		sandbox.stub(GuardianEnv, "config").returns({
			environment: Environment.production,
			appBaseUrl: "https://app.guardian-mock.bot",
			apiBaseUrl: "https://api.guardian-mock.bot",
			mcpBaseUrl: "https://api.guardian-mock.bot/v1/mcp",
		})
	...
	describe("IDE Type Detection", () => {
		...
		it('should return "jetbrains" when ide is "Guardian for JetBrains" (case-insensitive)', async () => {
			stubHostInfo({ ide: "Guardian for JetBrains" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
		...
		it("should prefer ide field over platform field for detection", async () => {
			stubHostInfo({ ide: "Guardian for JetBrains", platform: "Visual Studio Code 1.103.0" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
