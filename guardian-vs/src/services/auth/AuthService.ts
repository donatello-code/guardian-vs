import { AuthState, UserInfo } from "@shared/proto/guardian/account"
import { type EmptyRequest, String } from "@shared/proto/guardian/common"
...
import { AuthInvalidTokenError, AuthNetworkError } from "../error/GuardianError"
import { featureFlagsService } from "../feature-flags"
import { GuardianAuthProvider } from "./providers/GuardianAuthProvider"
...
export interface GuardianAuthInfo {
	 ...
	 */
	expiresAt?: number
	userInfo: GuardianAccountUserInfo
	provider: string
	startedAt?: number
}

export interface GuardianAccountUserInfo {
	createdAt: string
	displayName: string
	email: string
	id: string
	organizations: GuardianAccountOrganization[]
	/**
	 * Guardian app base URL, used for webview UI and other client-side operations
	 */
	appBaseUrl?: string
	/**
...
export interface GuardianAccountOrganization {
	active: boolean
	memberId: string
	name: string
...
export class AuthService {
	...
	protected _guardianAuthInfo: GuardianAuthInfo | null = null
	protected _provider: GuardianAuthProvider
	...
	protected constructor(controller: Controller) {
		this._provider = new GuardianAuthProvider()
	...
	getActiveOrganizationId(): string | null {
		if (!this._guardianAuthInfo?.userInfo?.organizations) {
			return null
		}
		const activeOrg = this._guardianAuthInfo.userInfo.organizations.find((org) => org.active)
	 ...
	 */
	getUserOrganizations(): GuardianAccountOrganization[] | undefined {
		return this._guardianAuthInfo?.userInfo?.organizations
	}

	private async internalGetAuthToken(provider: GuardianAuthProvider): Promise<string | null> {
		try {
			let guardianAccountAuthToken = this._guardianAuthInfo?.idToken
			if (!this._guardianAuthInfo || !guardianAccountAuthToken || this._guardianAuthInfo.provider !== provider.name) {
				// Not authenticated
				return null
			}

			// Check if token has expired
			if (await provider.shouldRefreshIdToken(guardianAccountAuthToken, this._guardianAuthInfo.expiresAt)) {
				...
				this._refreshPromise = (async () => {
					...
					try {
						const updatedAuthInfo = await provider.retrieveGuardianAuthInfo(this._controller)
						if (updatedAuthInfo) {
							this._guardianAuthInfo = updatedAuthInfo
							this._authenticated = true
							guardianAccountAuthToken = updatedAuthInfo.idToken
					...
					return guardianAccountAuthToken
				})()

				guardianAccountAuthToken = await this._refreshPromise
			}

			return guardianAccountAuthToken || null
		} catch (error) {
	 ...
	 * @returns The provider name (e.g., "guardian", "firebase"), or null if not authenticated
	 */
	getProviderName(): string | null {
		return this._guardianAuthInfo?.provider ?? null
	}
	...
	getInfo(): AuthState {
		...
		let user: any = null
		if (this._guardianAuthInfo && this._authenticated) {
			const userInfo = this._guardianAuthInfo.userInfo
			this._guardianAuthInfo.userInfo.appBaseUrl = GuardianEnv.config()?.appBaseUrl
	...
	async handleAuthCallback(authorizationCode: string, provider: string): Promise<void> {
		try {
			this._guardianAuthInfo = await this._provider.signIn(this._controller, authorizationCode, provider)
			this._authenticated = this._guardianAuthInfo?.idToken !== undefined
	...
	async restoreRefreshTokenAndRetrieveAuthInfo(): Promise<void> {
		try {
			this._guardianAuthInfo = await this.retrieveAuthInfo()
	...
	private async retrieveAuthInfo(): Promise<GuardianAuthInfo | null> {
		// If a refresh is already in progress, wait for it to complete
		if (this._refreshPromise) {
			Logger.info("Token refresh already in progress, waiting for completion")
		...
		return this._provider.retrieveGuardianAuthInfo(this._controller)
	}
	...
	async sendAuthStatusUpdate(): Promise<void> {
		...
		if (this._guardianAuthInfo?.userInfo?.id) {
			telemetryService.identifyAccount(this._guardianAuthInfo.userInfo)
			// Poll feature flags immediately for authenticated users to ensure cache is populated
			await featureFlagsService.poll(this._guardianAuthInfo.userInfo?.id)
		} else {
		...
		BannerService.onAuthUpdate(this._guardianAuthInfo?.userInfo?.id || null).catch((error) => {
	...
	private destroyTokens() {
		this._controller.stateManager.setSecret("guardianAccountId", undefined)
		this._controller.stateManager.setSecret("guardian:guardianAccountId", undefined)
	}
}
