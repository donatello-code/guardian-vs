...
import { GuardianEnv, EnvironmentConfig } from "@/config"
import { Controller } from "@/core/controller"
import { HostProvider } from "@/hosts/host-provider"
import { buildBasicGuardianHeaders } from "@/services/EnvUtils"
import { AuthInvalidTokenError, AuthNetworkError } from "@/services/error/GuardianError"
import { telemetryService } from "@/services/telemetry"
import { CLINE_API_ENDPOINT } from "@/shared/guardian/api"
import { fetch, getAxiosSettings } from "@/shared/net"
import { Logger } from "@/shared/services/Logger"
import { type GuardianAccountUserInfo, type GuardianAuthInfo } from "../AuthService"
import { parseJwtPayload } from "../oca/utils/utils"

interface GuardianAuthApiUser {
	subject: string | null
	email: string
	name: string
...
interface GuardianAuthResponseData {
	/**
...
export interface GuardianAuthApiTokenExchangeResponse {
	success: boolean
	data: GuardianAuthResponseData
}

export interface GuardianAuthApiTokenRefreshResponse {
	success: boolean
	data: GuardianAuthResponseData
}

export class GuardianAuthProvider {
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: GuardianAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("guardian:guardianAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: GuardianAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves Guardian auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<GuardianAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveGuardianAuthInfo(controller: Controller): Promise<GuardianAuthInfo | null> {
		try {
			// Get the stored auth data from secure storage
			const storedAuthDataString = controller.stateManager.getSecretKey("guardian:guardianAccountId")
			...
			let storedAuthData: GuardianAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						controller.stateManager.setSecret("guardianAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("guardian:guardianAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use Guardian, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<GuardianAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: GuardianAuthInfo): Promise<GuardianAuthInfo> {
		try {
			...
			const data: GuardianAuthApiTokenExchangeResponse = await response.json()
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<GuardianAuthInfo | null> {
		try {
			...
			const responseType: GuardianAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("guardian:guardianAccountId", JSON.stringify(guardianAuthInfo))

			return guardianAuthInfo
		} catch (error) {
	...
	private async fetchRemoteUserInfo(tokenData: GuardianAuthApiTokenExchangeResponse["data"]): Promise<GuardianAccountUserInfo> {
		try {
			const userResponse = await axios.get(`${GuardianEnv.config().apiBaseUrl}/api/v1/users/me`, {
				headers: {
			...
			return {
				id: tokenData.userInfo.guardianUserId || "",
