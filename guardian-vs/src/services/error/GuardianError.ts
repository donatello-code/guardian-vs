...
export class GuardianError extends Error {
	...
	/**
	 * Parses a stringified error into a GuardianError instance.
	 */
	static parse(errorStr?: string, modelId?: string): GuardianError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return GuardianError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a GuardianError instance.
	 * Always returns a GuardianError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): GuardianError {
		try {
			// If already a GuardianError, return it directly to prevent infinite recursion
			if (error instanceof GuardianError) {
				return error
			}
			return new GuardianError(JSON.parse(error), modelId, providerId)
		} catch {
			return new GuardianError(error, modelId, providerId)
		}
	}

	public isErrorType(type: GuardianErrorType): boolean {
		return GuardianError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: GuardianError): GuardianErrorType | undefined {
		...
		if (code === "insufficient_credits" && typeof details?.current_balance === "number") {
			return GuardianErrorType.Balance
		}
		...
		if (message) {
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return GuardianErrorType.RateLimit
			}
		}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = GuardianErrorType.Network
	}
}
...
export class AuthInvalidTokenError extends Error {
	constructor(message: string) {
		super(message)
		this.name = GuardianErrorType.Auth
	}
}
