...
/**
 * Implementation of OAuthClientProvider for Guardian
 * Manages OAuth state and token storage for a single MCP server
 */
class GuardianOAuthClientProvider implements OAuthClientProvider {
	...
	get clientMetadata(): OAuthClientMetadata {
		return {
			...
			client_uri: "https://guardian.bot",
			software_id: "guardian",
		}
	}
...
export class McpOAuthManager {
	...
	async getOrCreateProvider(serverName: string, serverUrl: string): Promise<OAuthClientProvider> {
		...
		// Create provider
		const provider = new GuardianOAuthClientProvider(serverName, serverUrl)
