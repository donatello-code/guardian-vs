...
export async function openFocusChainFile(controller: Controller, request: StringRequest): Promise<Empty> {
	...
	if (currentTask) {
		...
		// TODO - can we decouple this from GuardianMessages?
		const guardianMessages = currentTask.messageStateHandler.getGuardianMessages()
		const lastProgressMessage = guardianMessages
			.slice()
			.reverse()
