import { HookInfo, HooksToggles, WorkspaceHooks } from "@shared/proto/guardian/file"
...
export async function refreshHooks(
	...
	const globalHooksDir = globalHooksDirOverride || path.join(os.homedir(), "Documents", "Guardian", "Hooks")
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".guardianrules", "hooks")
