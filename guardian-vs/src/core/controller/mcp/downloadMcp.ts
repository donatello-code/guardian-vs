...
import { McpDownloadResponse } from "@shared/proto/guardian/mcp"
...
export async function downloadMcp(controller: Controller, request: StringRequest): Promise<McpDownloadResponse> {
	try {
		...
		const response = await axios.post<McpDownloadResponse>(
			`${GuardianEnv.config().mcpBaseUrl}/download`,
			{ mcpId },
			{
...
- Use "${mcpDetails.mcpId}" as the server name in guardian_mcp_settings.json.
- Create the directory for the new MCP server before starting installation.
- Make sure you read the user's existing guardian_mcp_settings.json file before editing it with this new mcp, to not overwrite any existing servers.
