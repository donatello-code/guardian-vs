...
import { WorktreeDefaults } from "@shared/proto/guardian/worktree"
...
export async function getWorktreeDefaults(_controller: Controller, _request: EmptyRequest): Promise<WorktreeDefaults> {
	...
	const suggestedBranch = `worktree/guardian-${suffix}`

	// Generate suggested path in Documents/Guardian/Worktrees/<project-name>-<suffix>
	...
	const suggestedPath = path.join(documentsPath, "Guardian", "Worktrees", `${projectName}-${suffix}`)
