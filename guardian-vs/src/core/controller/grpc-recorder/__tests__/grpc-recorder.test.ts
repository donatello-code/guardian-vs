		...
		it("using default filtering should filter out unwanted requests", async () => {
			const customRecorder = GrpcRecorder.builder()
				.withFilters(
					...
					(req) => ["guardian.UiService", "guardian.McpService", "guardian.WebService"].includes(req.service),
				)
				.enableIf(true)
				.build()
			const unwantedServices = ["guardian.UiService", "guardian.McpService", "guardian.WebService"]
