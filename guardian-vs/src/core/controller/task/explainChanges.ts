...
import { ExplainChangesRequest } from "@shared/proto/guardian/task"
...
export async function explainChanges(controller: Controller, request: ExplainChangesRequest): Promise<Empty> {
	...
	try {
		...
		const messageIndex = guardianMessages.findIndex((m: any) => m.ts === request.messageTs)
		...
		const lastTaskCompletedMessageCheckpointHash = findLast(
			guardianMessages.slice(0, messageIndex),
			(m: any) => m.say === "completion_result",
		)?.lastCheckpointHash

		const firstCheckpointMessageCheckpointHash = guardianMessages.find(
