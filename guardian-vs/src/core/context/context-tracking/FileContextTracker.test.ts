...
describe("FileContextTracker", () => {
	...
	it("should add a record when a file is read by a tool", async () => {
		...
		expect(fileEntry.guardian_read_date).to.be.a("number")
		expect(fileEntry.guardian_edit_date).to.be.null
	})

	it("should add a record when a file is edited by Guardian", async () => {
		await tracker.trackFileContext(filePath, "guardian_edited")
		...
		expect(activeEntry.record_source).to.equal("guardian_edited")
		expect(activeEntry.guardian_read_date).to.be.a("number")
		expect(activeEntry.guardian_edit_date).to.be.a("number")
	})
	...
	it("should mark existing entries as stale when adding a new entry for the same file", async () => {
		...
		mockTaskMetadata.files_in_context = [
			{
				...
				guardian_read_date: Date.now() - 1000, // 1 second ago
				guardian_edit_date: null,
				user_edit_date: null,
			},
		]
		...
		expect(newEntry.record_source).to.equal("guardian_edited")
	})
	...
	it("should not track Guardian edits as user edits", async () => {
		// First track the file to set up the watcher
		await tracker.trackFileContext(filePath, "read_tool")

		// Mark the file as edited by Guardian
		tracker.markFileAsEditedByGuardian(filePath)
