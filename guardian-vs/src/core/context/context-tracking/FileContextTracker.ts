...
// If a user modifies a file outside of Guardian, the context may become stale and need to be updated.
// We do not want Guardian to reload the context every time a file is modified, so we use this class merely
// to inform Guardian that the change has occurred, and tell Guardian to reload the file before making
// any changes to it. This fixes an issue with diff editing, where Guardian was unable to complete a diff edit.
// a diff edit because the file was modified since Guardian last read it.

// FileContextTracker
/**
This class is responsible for tracking file operations.
If the full contents of a file are passed to Guardian via a tool, mention, or edit, the file is marked as active.
If a file is modified outside of Guardian, we detect and track this change to prevent stale context.
...
export class FileContextTracker {
	...
	private recentlyEditedByGuardian = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.on("change", () => {
			if (this.recentlyEditedByGuardian.has(filePath)) {
				this.recentlyEditedByGuardian.delete(filePath) // This was an edit by Guardian, no need to inform Guardian
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform Guardian
	 ...
	 * This is the main entry point for FileContextTracker and is called when a file is passed to Guardian via a tool, mention, or edit.
	 */
	async trackFileContext(filePath: string, operation: "read_tool" | "user_edited" | "guardian_edited" | "file_mentioned") {
		try {
			const cwd = await getCwd()
			if (!cwd) {
				Logger.info("No workspace folder available - cannot determine current working directory")
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: FileMetadataEntry["record_source"]) {
		try {
			...
			const newEntry: FileMetadataEntry = {
				...
				guardian_read_date: getLatestDateForField(filePath, "guardian_read_date"),
				guardian_edit_date: getLatestDateForField(filePath, "guardian_edit_date"),
			...
			switch (source) {
				...
				// guardian_edited: Guardian has edited the file
				case "guardian_edited":
					newEntry.guardian_read_date = now
					newEntry.guardian_edit_date = now
					break

				// read_tool/file_mentioned: Guardian has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.guardian_read_date = now
					break
	...
	/**
	 * Marks a file as edited by Guardian to prevent false positives in file watchers
	 */
	markFileAsEditedByGuardian(filePath: string): void {
		this.recentlyEditedByGuardian.add(filePath)
	}
	...
	/**
	 * Detects files that were edited by Guardian or users after a specific message timestamp
	 * This is used when restoring checkpoints to warn about potential file content mismatches
	 */
	async detectFilesEditedAfterMessage(messageTs: number, deletedMessages: GuardianMessage[]): Promise<string[]> {
		const editedFiles: string[] = []

		try {
			// Check task metadata for files that were edited by Guardian or users after the message timestamp
			...
			if (taskMetadata?.files_in_context) {
				for (const fileEntry of taskMetadata.files_in_context) {
					const guardianEditedAfter = fileEntry.guardian_edit_date && fileEntry.guardian_edit_date > messageTs
					const userEditedAfter = fileEntry.user_edit_date && fileEntry.user_edit_date > messageTs

					if (guardianEditedAfter || userEditedAfter) {
