...
type MessageStateHandlerLike = {
	getGuardianMessages(): GuardianMessageLike[]
}
...
/**
 * Builds the evaluation context used for conditional Guardian Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in guardianMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const guardianMessages = deps.messageStateHandler.getGuardianMessages()
		...
		const lastUserMsg = [...guardianMessages]
			.reverse()
		...
		// (3) Files edited by Guardian during this task (completed operations):
