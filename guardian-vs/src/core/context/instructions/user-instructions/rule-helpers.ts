...
export async function synchronizeRuleToggles(
	...
	currentToggles: GuardianRulesToggles,
	allowedFileExtension: string = "",
	excludedPaths: string[][] = [],
): Promise<GuardianRulesToggles> {
...
export function synchronizeRemoteRuleToggles(
	...
	const updatedToggles: GuardianRulesToggles = {}
 ...
 */
export function combineRuleToggles(toggles1: GuardianRulesToggles, toggles2: GuardianRulesToggles): GuardianRulesToggles {
 ...
 */
export const getRuleFilesTotalContent = async (rulesFilePaths: string[], basePath: string, toggles: GuardianRulesToggles) => {
 ...
 * Used by getGlobalGuardianRules and getLocalGuardianRules.
 */
...
export function getRemoteRulesTotalContentWithMetadata(
	remoteRules: GlobalInstructionsFile[],
	remoteToggles: GuardianRulesToggles,
...
/**
 * Handles converting any directory into a file (specifically used for .guardianrules and .guardianrules/workflows)
 * The old .guardianrules file or .guardianrules/workflows file will be renamed to a default filename
 * Doesn't do anything if the dir already exists or doesn't exist
 * Returns whether there are any uncaught errors
 */
export async function ensureLocalGuardianDirExists(guardianrulePath: string, defaultRuleFilename: string): Promise<boolean> {
	try {
		const exists = await fileExistsAtPath(guardianrulePath)

		if (exists && !(await isDirectory(guardianrulePath))) {
			// logic to convert .guardianrules file into directory, and rename the rules file to {defaultRuleFilename}
			const content = await fs.readFile(guardianrulePath, "utf8")
			const tempPath = guardianrulePath + ".bak"
			await fs.rename(guardianrulePath, tempPath) // create backup
			try {
				await fs.mkdir(guardianrulePath, { recursive: true })
				await fs.writeFile(path.join(guardianrulePath, defaultRuleFilename), content, "utf8")
				...
				try {
					await fs.rm(guardianrulePath, { recursive: true, force: true }).catch(() => {})
					await fs.rename(tempPath, guardianrulePath) // restore backup
				} catch (_restoreError) {}
...
export const createRuleFile = async (isGlobal: boolean, filename: string, cwd: string, type: string) => {
	try {
		...
		if (isGlobal) {
			if (type === "workflow") {
				const globalGuardianWorkflowFilePath = await ensureWorkflowsDirectoryExists()
				filePath = path.join(globalGuardianWorkflowFilePath, filename)
			} else {
				const globalGuardianRulesFilePath = await ensureRulesDirectoryExists()
				filePath = path.join(globalGuardianRulesFilePath, filename)
			}
		} else {
			const localGuardianRulesFilePath = path.resolve(cwd, GlobalFileNames.guardianRules)

			const hasError = await ensureLocalGuardianDirExists(localGuardianRulesFilePath, "default-rules.md")
			if (hasError === true) {
				return { filePath: null, fileExists: false }
			}

			await fs.mkdir(localGuardianRulesFilePath, { recursive: true })

			if (type === "workflow") {
				const localWorkflowsFilePath = path.resolve(cwd, GlobalFileNames.workflows)

				const hasError = await ensureLocalGuardianDirExists(localWorkflowsFilePath, "default-workflows.md")
				if (hasError === true) {
					return { filePath: null, fileExists: false }
				}
				...
				// guardianrules file creation
				filePath = path.join(localGuardianRulesFilePath, filename)
			}
		}
