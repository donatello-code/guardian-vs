 ...
 */
export function extractTokenUsageFromMessage(message: GuardianMessage | undefined): TokenUsage {
...
export async function writePreCompactContextFiles(
	taskId: string,
	currentContext: GuardianStorageMessage[],
...
export interface PreCompactHookParams {
	...
	apiConversationHistory: GuardianStorageMessage[]
	/** Current deleted range (if any) */
	conversationHistoryDeletedRange?: [number, number]
	/** Guardian messages for extracting token usage */
...
export async function executePreCompactHookWithCleanup(params: PreCompactHookParams): Promise<PreCompactHookResult> {
	...
	try {
		...
		const previousApiReqIndex = findLastIndex(params.guardianMessages, (m) => m.say === "api_req_started")
		const previousRequest = previousApiReqIndex !== -1 ? params.guardianMessages[previousApiReqIndex] : undefined
		...
		if (preCompactResult.cancel === true) {
			...
			await params.messageStateHandler.saveGuardianMessagesAndUpdateHistory()
