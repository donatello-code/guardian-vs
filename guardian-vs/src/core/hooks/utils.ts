...
/**
 * All valid hook types that can be created and executed by Guardian.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Guardian", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".guardianrules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".guardianrules", "hooks")
}
