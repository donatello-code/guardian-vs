...
import type { HookOutput } from "@shared/proto/guardian/hooks"
...
async function updateHookMessage(
	...
	const guardianMessages = messageStateHandler.getGuardianMessages()
	const hookMessageIndex = guardianMessages.findIndex((m: GuardianMessage) => m.ts === hookMessageTs)
	if (hookMessageIndex !== -1) {
		await messageStateHandler.updateGuardianMessage(hookMessageIndex, {
...
async function reorderHookAndToolMessages(messageStateHandler: MessageStateHandler): Promise<void> {
	...
	for (let i = guardianMessages.length - 1; i >= 0; i--) {
		const msgType = guardianMessages[i].ask || guardianMessages[i].say
	...
	for (let i = lastToolMessageIndex + 1; i < guardianMessages.length; i++) {
		if (guardianMessages[i].say === "hook_status" || guardianMessages[i].say === "hook_output_stream") {
	...
	const toolMessage = { ...guardianMessages[lastToolMessageIndex] }

	// Delete the tool message at its current position
	await messageStateHandler.deleteGuardianMessage(lastToolMessageIndex)

	// Re-add the tool message at the end (after hook messages)
	await messageStateHandler.addToGuardianMessages(toolMessage)
}
