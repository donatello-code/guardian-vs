...
export async function parseSlashCommands(
...
): Promise<{ processedText: string; needsGuardianrulesFileCheck: boolean }> {
	...
	for (const { regex } of tagPatterns) {
		...
		if (tagMatch) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsGuardianrulesFileCheck: commandName === "newrule" }
			}
				...
				if (parts.length >= 3) {
					...
					try {
						...
						if (promptResponse) {
							...
							return { processedText, needsGuardianrulesFileCheck: false }
						}
	...
	return { processedText: text, needsGuardianrulesFileCheck: false }
}
