 ...
 * Designed to be instantiated once in Guardian.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .guardianignore files.
 */
export class GuardianIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	guardianIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.guardianIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .guardianignore
		this.setupFileWatcher()
		await this.loadGuardianIgnore()
	}

	/**
	 * Set up the file watcher for .guardianignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".guardianignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .guardianignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .guardianignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadGuardianIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.guardianIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".guardianignore")
			} else {
				this.guardianIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .guardianignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processGuardianIgnoreIncludes(content)
	 ...
	 */
	private async processGuardianIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[GuardianIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .guardianignore does not exist
		if (!this.guardianIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .guardianignore exists
		if (!this.guardianIgnoreContent) {
			return undefined
		}
