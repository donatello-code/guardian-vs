...
import { GuardianIgnoreController } from "./GuardianIgnoreController"
import "should"

describe("GuardianIgnoreController", () => {
	let tempDir: string
	let controller: GuardianIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .guardianignore file
		await fs.writeFile(
			path.join(tempDir, ".guardianignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new GuardianIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .guardianignore file", async () => {
			const result = controller.validateAccess(".guardianignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .guardianignore", async () => {
			// Create a new .guardianignore with comments
			await fs.writeFile(
				path.join(tempDir, ".guardianignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .guardianignore gracefully", async () => {
			// Create a new controller in a directory without .guardianignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new GuardianIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .guardianignore", async () => {
			await fs.writeFile(path.join(tempDir, ".guardianignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .guardianignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".guardianignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .guardianignore
			controller = new GuardianIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .guardianignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .guardianignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".guardianignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".guardianignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
