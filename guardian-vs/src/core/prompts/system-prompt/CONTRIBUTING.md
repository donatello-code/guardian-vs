...
**Location:** [`proto/guardian/models.proto`](../../../proto/guardian/models.proto)
...
2. **Tool Variant Fallback:**
   ...
   - Handled by `GuardianToolSet.getToolByNameWithFallback()`
...
export const config = createVariant(ModelFamily.NEXT_GEN)
    ...
    .tools(
        ...
        GuardianDefaultTool.MCP_USE,  // Instead of MCP_ACCESS
        // ... other tools
    )
```
...
API formats are defined in [`proto/guardian/models.proto`](../../../proto/guardian/models.proto):
...
1. **Add to proto:** [`proto/guardian/models.proto`](../../../proto/guardian/models.proto)
2. **Regenerate:** `npm run protos`
3. **Import:** `import { ApiFormat } from "@/shared/proto/guardian/models"`
...
**Default behavior:** Tools automatically fall back to `GENERIC` variant via `GuardianToolSet.getToolByNameWithFallback()`.
...
const id = GuardianDefaultTool.FILE_NEW
...
export function registerGuardianToolSets(): void {
    ...
    allToolVariants.forEach((v) => GuardianToolSet.register(v))
}
```
...
.tools(
    GuardianDefaultTool.BASH,
    GuardianDefaultTool.FILE_NEW,  // Add your tool here
    // ... other tools
)
```
...
**Note:** When a variant includes a tool in `.tools()` but the tool doesn't have a specific variant for that model family, the system automatically uses the `GENERIC` variant. This is handled by `GuardianToolSet.getToolByNameWithFallback()`, so you don't need to manually define variants for every model family—only when behavior needs to differ.
...
**For live testing with real models**, run Guardian in debug mode to verify your variant works correctly:

1. **Enable Debug Mode:**
   - See the main [CONTRIBUTING.md](../../../../CONTRIBUTING.md) for instructions on running Guardian in debug mode
   - Debug mode enables additional features for testing and verification

2. **Run a Task with Your Model:**
   - Configure your model in Guardian settings
...
- **Proto Definitions:** [`proto/guardian/models.proto`](../../../proto/guardian/models.proto)
...
│   └── tools.ts                # GuardianDefaultTool enum
├── utils/
