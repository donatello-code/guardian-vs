...
import { type GuardianToolSpec, toolSpecFunctionDeclarations, toolSpecFunctionDefinition, toolSpecInputSchema } from "../spec"
import { PromptVariant, SystemPromptContext } from "../types"

export class GuardianToolSet {
	// A list of tools mapped by model group
	private static variants: Map<ModelFamily, Set<GuardianToolSet>> = new Map()

	private constructor(
		public readonly id: string,
		public readonly config: GuardianToolSpec,
	) {
		this._register()
	}

	public static register(config: GuardianToolSpec): GuardianToolSet {
		return new GuardianToolSet(config.id, config)
	}

	private _register(): void {
		const existingTools = GuardianToolSet.variants.get(this.config.variant) || new Set()
		if (!Array.from(existingTools).some((t) => t.config.id === this.config.id)) {
			existingTools.add(this)
			GuardianToolSet.variants.set(this.config.variant, existingTools)
		}
	}

	public static getTools(variant: ModelFamily): GuardianToolSet[] {
	...
	public static getRegisteredModelIds(): string[] {
		return Array.from(GuardianToolSet.variants.keys())
	}

	public static getToolByName(toolName: string, variant: ModelFamily): GuardianToolSet | undefined {
		const tools = GuardianToolSet.getTools(variant)
	...
	public static getToolByNameWithFallback(toolName: string, variant: ModelFamily): GuardianToolSet | undefined {
		// Try exact variant first
		const exact = GuardianToolSet.getToolByName(toolName, variant)
		if (exact) {
			return exact
		}

		// Fallback to GENERIC
		const generic = GuardianToolSet.getToolByName(toolName, ModelFamily.GENERIC)
		if (generic) {
			return generic
		}

		// Final fallback: search across all registered variants
		for (const [, tools] of GuardianToolSet.variants) {
	...
	public static getToolsForVariantWithFallback(variant: ModelFamily, requestedIds: string[]): GuardianToolSet[] {
		const resolved: GuardianToolSet[] = []
		for (const id of requestedIds) {
			const tool = GuardianToolSet.getToolByNameWithFallback(id, variant)
			if (tool) {
	...
	public static getEnabledTools(variant: PromptVariant, context: SystemPromptContext): GuardianToolSet[] {
		const resolved: GuardianToolSet[] = []
		const requestedIds = variant.tools ? [...variant.tools] : []
		for (const id of requestedIds) {
			const tool = GuardianToolSet.getToolByNameWithFallback(id, variant.family)
			if (tool) {
				// Avoid duplicates by id
				if (!resolved.some((t) => t.config.id === tool.config.id)) {
					resolved.push(tool)
	...
	public static getNativeTools(variant: PromptVariant, context: SystemPromptContext) {
		...
		// Base set
		const toolsets = GuardianToolSet.getEnabledTools(variant, context)
		const toolConfigs = toolsets.map((tool) => tool.config)

		// MCP tools
		const mcpServers = context.mcpHub?.getServers()?.filter((s) => s.disabled !== true) || []
		const mcpTools = mcpServers?.flatMap((server) => mcpToolToGuardianToolSpec(variant.family, server))
...
/**
 * Convert an MCP server's tools to GuardianToolSpec format
 */
export function mcpToolToGuardianToolSpec(family: ModelFamily, server: McpServer): GuardianToolSpec[] {
	...
	return tools
		.map((mcpTool) => {
			...
			if (mcpToolName?.length <= 64) {
				return {
					variant: family,
					id: GuardianDefaultTool.MCP_USE,
