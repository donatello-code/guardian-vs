...
import { type GuardianToolSpec, resolveInstruction } from "../spec"
...
export class PromptBuilder {
	...
	private static getEnabledTools(variant: PromptVariant, context: SystemPromptContext) {
		let resolvedTools: ReturnType<typeof GuardianToolSet.getTools> = []
		...
		if (variant?.tools?.length) {
			...
			resolvedTools = GuardianToolSet.getToolsForVariantWithFallback(variant.family, requestedIds)
			...
			resolvedTools = GuardianToolSet.getTools(variant.family)
	...
	public static tool(config: GuardianToolSpec, registry: GuardianDefaultTool[], context: SystemPromptContext): string {
