...
This directory contains the tool registration system for Guardian tools. The system automatically collects and registers all tool variants with the `GuardianToolSet` provider.

## Overview

Each tool file in this directory exports a `{toolName}_variants` array containing tool specifications for different prompt variants (e.g., Claude, GPT). The registration system automatically imports all these variants and registers them with the `GuardianToolSet` provider.
...
```typescript
import { GuardianToolSet } from "../registry/GuardianToolSet";
import { PromptVariant } from "@/shared/tools";

// Get all tools for a specific variant
const claudeTools = GuardianToolSet.getTools(PromptVariant.CLAUDE);

// Get a specific tool by name
const writeToFileTool = GuardianToolSet.getToolByName("write_to_file", PromptVariant.CLAUDE);
```
...
```typescript
import { GuardianDefaultTool, PromptVariant, type GuardianToolSpec } from "@/shared/tools";
