...
├── registry/
│   ├── GuardianToolSet.ts            # Tool set management & registry
...
interface PromptVariant {
  ...
  // Tool configuration
  tools?: GuardianDefaultTool[];                    // Ordered list of tools to include
  toolOverrides?: { [K in GuardianDefaultTool]?: ConfigOverride }; // Tool-specific customizations
}
...
```markdown
You are Guardian, a highly skilled software engineer...
...
Tools are managed through the `GuardianToolSet` and can be configured per variant:

```typescript
class GuardianToolSet {
  private static variants: Map<ModelFamily, Set<GuardianToolSet>> = new Map();

  static register(config: GuardianToolSpec): GuardianToolSet {
    return new GuardianToolSet(config.id, config);
  }

  static getTools(variant: ModelFamily): GuardianToolSet[] {
    const toolsSet = GuardianToolSet.variants.get(variant) || new Set();
    const defaultSet = GuardianToolSet.variants.get(ModelFamily.GENERIC) || new Set();
...
public static async getToolsPrompts(variant: PromptVariant, context: SystemPromptContext) {
  const tools = GuardianToolSet.getTools(variant.family);
...
export const config = createVariant(ModelFamily.NEXT_GEN)
  ...
  .tools(
    ...
    GuardianDefaultTool.LIST_CODE_DEF,
    GuardianDefaultTool.BROWSER,
    GuardianDefaultTool.WEB_FETCH,  // Additional tool for next-gen
    GuardianDefaultTool.MCP_USE,
    GuardianDefaultTool.MCP_ACCESS,
    GuardianDefaultTool.ASK,
    GuardianDefaultTool.ATTEMPT,
...
const config = createVariant(ModelFamily.GENERIC)
  ...
  .overrideTool(GuardianDefaultTool.BASH, {          // Optional, tool overrides
    enabled: false,
  })
...
The system supports the following tools (mapped to `GuardianDefaultTool` enum):
...
Each tool in Guardian follows a specific structure with variants for different model families. Here's the anatomy of a tool:
...
const id = GuardianDefaultTool.MY_NEW_TOOL // Add to enum first
...
const generic: GuardianToolSpec = {
	...
	parameters: [
		...
		},
		{
			...
			dependencies: [GuardianDefaultTool.SOME_OTHER_TOOL], // Only show if dependency exists
		},
	],
}
...
First, add your tool ID to the `GuardianDefaultTool` enum:
...
const id = GuardianDefaultTool.MY_NEW_TOOL
...
export function registerGuardianToolSets(): void {
...
```typescript
// In your tool handler class (e.g., GuardianProvider)
...
```typescript
const contextAwareTool: GuardianToolSpec = {
	variant: ModelFamily.GENERIC,
	id: GuardianDefaultTool.CONTEXT_TOOL,
	name: "context_tool",
...
```typescript
const claude: GuardianToolSpec = {
	variant: ModelFamily.GENERIC,
	id: GuardianDefaultTool.MODEL_SPECIFIC_TOOL,
...
```typescript
const dependentTool: GuardianToolSpec = {
	variant: ModelFamily.GENERIC,
	id: GuardianDefaultTool.DEPENDENT_TOOL,
	name: "dependent_tool",
	...
	parameters: [
		...
		},
		{
			...
			dependencies: [GuardianDefaultTool.TODO],
		},
	],
}
```
...
const id = GuardianDefaultTool.ANALYZE_FILE
...
const nextGen: GuardianToolSpec = {
	...generic,
...
This comprehensive guide should help developers understand both the architecture and practical steps needed to extend Guardian with new tools.
