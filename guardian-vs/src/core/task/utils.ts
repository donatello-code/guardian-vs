...
import { GuardianApiReqCancelReason, GuardianApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const guardianMessages = params.messageStateHandler.getGuardianMessages()
	const currentApiReqInfo: GuardianApiReqInfo = JSON.parse(guardianMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateGuardianMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
