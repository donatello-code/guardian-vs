...
export interface PendingReasoning {
	...
	content: string
	signature: string
	redactedThinking: GuardianAssistantRedactedThinkingBlock[]
...
/**
 * Handles streaming native tool use blocks and converts them to GuardianAssistantToolUseBlock format
 */
class ToolUseHandler {
	...
	getFinalizedToolUse(id: string): GuardianAssistantToolUseBlock | undefined {
	...
	getAllFinalizedToolUses(summary?: GuardianAssistantToolUseBlock["reasoning_details"]): GuardianAssistantToolUseBlock[] {
		const results: GuardianAssistantToolUseBlock[] = []
	...
	getPartialToolUsesAsContent(): ToolUse[] {
		...
		for (const pending of pendingToolUses) {
			...
			if (pending.name.includes(CLINE_MCP_TOOL_IDENTIFIER)) {
				...
				results.push({
					type: "tool_use",
					name: GuardianDefaultTool.MCP_USE,
					params: {
				...
				results.push({
					type: "tool_use",
					name: pending.name as GuardianDefaultTool,
...
class ReasoningHandler {
	...
	getCurrentReasoning(): GuardianAssistantThinkingBlock | null {
		if (!this.pendingReasoning) {
			return null
		}
	...
	getRedactedThinking(): GuardianAssistantRedactedThinkingBlock[] {
