...
import {
	getGlobalGuardianRules,
	getLocalGuardianRules,
	refreshGuardianRulesToggles,
} from "@core/context/instructions/user-instructions/guardian-rules"
import {
	getLocalAgentsRules,
	getLocalCursorRules,
...
import { GuardianApiReqCancelReason, GuardianApiReqInfo, GuardianAsk, GuardianMessage, GuardianSay } from "@shared/ExtensionMessage"
import { HistoryItem } from "@shared/HistoryItem"
import { DEFAULT_LANGUAGE_SETTINGS, getLanguageKey, LanguageDisplay } from "@shared/Languages"
import { USER_CONTENT_TAGS } from "@shared/messages/constants"
import { convertGuardianMessageToProto } from "@shared/proto-conversions/guardian-message"
import { GuardianDefaultTool, READ_ONLY_TOOLS } from "@shared/tools"
...
import { GuardianError, GuardianErrorType, ErrorService } from "@/services/error"
...
import { ensureLocalGuardianDirExists } from "../context/instructions/user-instructions/rule-helpers"
...
export type ToolResponse = GuardianToolResponseContent
...
export class Task {
	...
	constructor(params: TaskParams) {
		...
		this.guardianIgnoreController = new GuardianIgnoreController(cwd)
		...
		const effectiveApiConfiguration: ApiConfiguration = {
			...
			onRetryAttempt: async (attempt: number, maxRetries: number, delay: number, error: any) => {
				const guardianMessages = this.messageStateHandler.getGuardianMessages()
				const lastApiReqStartedIndex = findLastIndex(guardianMessages, (m) => m.say === "api_req_started")
				if (lastApiReqStartedIndex !== -1) {
					try {
						const currentApiReqInfo: GuardianApiReqInfo = JSON.parse(guardianMessages[lastApiReqStartedIndex].text || "{}")
						...
						await this.messageStateHandler.updateGuardianMessage(lastApiReqStartedIndex, {
		...
		const commandExecutorCallbacks: CommandExecutorCallbacks = {
			...
			ask: async (type: string, text?: string, partial?: boolean) => {
				const result = await this.ask(type as GuardianAsk, text, partial)
				return {
			...
			updateGuardianMessage: async (index: number, updates: { commandCompleted?: boolean; text?: string }) => {
				await this.messageStateHandler.updateGuardianMessage(index, updates)
			},
			getGuardianMessages: () => this.messageStateHandler.getGuardianMessages() as Array<{ ask?: string; say?: string }>,
			addToUserMessageContent: (content: { type: string; text: string }) => {
				// Cast to GuardianTextContentBlock which is compatible with GuardianContent
				this.taskState.userMessageContent.push({ type: "text", text: content.text } as GuardianTextContentBlock)
			},
		}
	...
	async ask(
		...
		if (this.taskState.abort && type !== "resume_task" && type !== "resume_completed_task") {
			throw new Error("Guardian instance aborted")
		}
		
		// Reset auto-completion timer when user provides input
		this.resetAutoCompletionTimer()
		
		let askTs: number
		if (partial !== undefined) {
			const guardianMessages = this.messageStateHandler.getGuardianMessages()
			const lastMessage = guardianMessages.at(-1)
			const lastMessageIndex = guardianMessages.length - 1
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.messageStateHandler.updateGuardianMessage(lastMessageIndex, {
						text,
						partial,
					})
					// todo be more efficient about saving and posting only new data or one whole message at a time so ignore partial for saves, and only post parts of partial message instead of whole array in new listener
					// await this.saveGuardianMessagesAndUpdateHistory()
					// await this.postStateToWebview()
					const protoMessage = convertGuardianMessageToProto(lastMessage)
				...
				await this.messageStateHandler.addToGuardianMessages({
					ts: askTs,
					type: "ask",
					ask: type,
					text,
			...
			// const lastMessage = this.guardianMessages.at(-1)
			this.taskState.askResponse = undefined
			this.taskState.askResponseText = undefined
			this.taskState.askResponseImages = undefined
			this.taskState.askResponseFiles = undefined
	...
	async handleWebviewAskResponse(askResponse: GuardianAskResponse, text?: string, images?: string[], files?: string[]) {
	...
	async say(
		...
		const modelInfo: GuardianMessageModelInfo = {
		...
		if (partial !== undefined) {
			const lastMessage = this.messageStateHandler.getGuardianMessages().at(-1)
			const isUpdatingPreviousPartial =
				lastMessage && lastMessage.partial && lastMessage.type === "say" && lastMessage.say === type
			if (partial) {
				if (isUpdatingPreviousPartial) {
					// existing partial message, so update it
					const lastIndex = this.messageStateHandler.getGuardianMessages().length - 1
					await this.messageStateHandler.updateGuardianMessage(lastIndex, {
						text,
						images,
						files,
						partial,
			...
			if (isUpdatingPreviousPartial) {
				...
				const lastIndex = this.messageStateHandler.getGuardianMessages().length - 1
				// updateGuardianMessage emits the change event and saves to disk
				await this.messageStateHandler.updateGuardianMessage(lastIndex, {
					text,
					images,
					files,
	...
	async sayAndCreateMissingParamError(toolName: GuardianDefaultTool, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`Guardian tried to use ${toolName}${
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: GuardianAsk | GuardianSay) {
		const guardianMessages = this.messageStateHandler.getGuardianMessages()
		const lastMessage = guardianMessages.at(-1)
		if (lastMessage?.partial && lastMessage.type === type && (lastMessage.ask === askOrSay || lastMessage.say === askOrSay)) {
			this.messageStateHandler.setGuardianMessages(guardianMessages.slice(0, -1))
			await this.messageStateHandler.saveGuardianMessagesAndUpdateHistory()
		}
	}
	 ...
	 */
	private calculatePreCompactDeletedRange(apiConversationHistory: GuardianStorageMessage[]): [number, number] {
	...
	public async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		try {
			await this.guardianIgnoreController.initialize()
		} catch (error) {
			Logger.error("Failed to initialize GuardianIgnoreController:", error)
			// Optionally, inform the user or handle the error appropriately
		}
		// conversationHistory (for API) and guardianMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the guardianMessages might not be empty, so we need to set it to [] when we create a new Guardian client (otherwise webview would show stale messages from previous session)
		this.messageStateHandler.setGuardianMessages([])
		...
		const imageBlocks: GuardianImageContentBlock[] = formatResponse.imageBlocks(images)

		const userContent: GuardianUserContent[] = [
			{
				type: "text",
				text: `<task>\n${task}\n</task>`,
			},
	...
	public async resumeTaskFromHistory() {
		...
		const savedGuardianMessages = await getSavedGuardianMessages(this.taskId)

		// Remove any resume messages that may have been added before

		const lastRelevantMessageIndex = findLastIndex(
			savedGuardianMessages,
			(m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"),
		)
		if (lastRelevantMessageIndex !== -1) {
			savedGuardianMessages.splice(lastRelevantMessageIndex + 1)
		}

		// since we don't use api_req_finished anymore, we need to check if the last api_req_started has a cost value, if it doesn't and no cancellation reason to present, then we remove it since it indicates an api request without any partial content streamed
		const lastApiReqStartedIndex = findLastIndex(savedGuardianMessages, (m) => m.type === "say" && m.say === "api_req_started")
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = savedGuardianMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: GuardianApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
			if (cost === undefined && cancelReason === undefined) {
				savedGuardianMessages.splice(lastApiReqStartedIndex, 1)
			}
		}

		await this.messageStateHandler.overwriteGuardianMessages(savedGuardianMessages)
		this.messageStateHandler.setGuardianMessages(await getSavedGuardianMessages(this.taskId))

		// Now present the guardian messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldn't be initialized when opening a old task, and it was because we were waiting for resume)
		...
		const lastGuardianMessage = this.messageStateHandler
			.getGuardianMessages()
			.slice()
			.reverse()
			.find((m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task")) // could be multiple resume tasks

		let askType: GuardianAsk
		if (lastGuardianMessage?.ask === "completion_result") {
		...
		const newUserContent: GuardianContent[] = []
			...
			const taskResumeResult = await executeHook({
				...
				hookInput: {
					taskResume: {
						...
						previousState: {
							lastMessageTs: lastGuardianMessage?.ts?.toString() || "",
							messageCount: guardianMessages.length.toString(),
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with guardian messages
		...
		let modifiedOldUserContent: GuardianContent[] // either the last message if its user message, or the user message before the last (assistant) message
		let modifiedApiConversationHistory: GuardianStorageMessage[] // need to remove the last user message to replace with new modified user message
		if (existingApiConversationHistory.length > 0) {
			...
			if (lastMessage.role === "assistant") {
				...
				const existingUserContent: GuardianContent[] = Array.isArray(lastMessage.content)
					? lastMessage.content
		...
		const agoText = (() => {
			const timestamp = lastGuardianMessage?.ts ?? Date.now()
			const now = Date.now()
		...
		const wasRecent = lastGuardianMessage?.ts && Date.now() - lastGuardianMessage.ts < 30_000
	...
	private async initiateTaskLoop(userContent: GuardianContent[]): Promise<void> {
		let nextUserContent = userContent
		let includeFileDetails = true
		while (!this.taskState.abort) {
			const didEndLoop = await this.recursivelyMakeGuardianRequests(nextUserContent, includeFileDetails)
			includeFileDetails = false // we only need file details the first time

			//  The way this agentic loop works is that guardian will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			...
			// this.say(
			// 	"tool",
			// 	"Guardian responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
			// )
			nextUserContent = [
				{
					type: "text",
	...
	async abortTask() {
		try {
			...
			this.guardianIgnoreController.dispose()
	...
	): Promise<[boolean, GuardianToolResponseContent]> {
	...
	public async cancelHookExecution(): Promise<boolean> {
		...
		try {
			...
			const guardianMessages = this.messageStateHandler.getGuardianMessages()
			const hookMessageIndex = guardianMessages.findIndex((m) => m.ts === messageTs)
			if (hookMessageIndex !== -1) {
				...
				await this.messageStateHandler.updateGuardianMessage(hookMessageIndex, {
	...
	private async writePromptMetadataArtifacts(params: { systemPrompt: string; providerInfo: ApiProviderInfo }): Promise<void> {
		...
		try {
			...
			const artifactDir = configuredDir
				...
				: path.resolve(this.cwd, ".guardian-prompt-artifacts")
		...
		if (hooksEnabled) {
			try {
				...
				await executePreCompactHookWithCleanup({
					...
					guardianMessages: this.messageStateHandler.getGuardianMessages(),
					messageStateHandler: this.messageStateHandler,
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const isCliEnvironment = host.guardianType === GuardianClient.Cli
		const browserSettings = this.stateManager.getGlobalSettingsKey("browserSettings")
		const disableBrowserTool = browserSettings.disableToolUse ?? false
		// guardian browser tool uses image recognition for navigation (requires model image support).
		...
		const { globalToggles, localToggles } = await refreshGuardianRulesToggles(this.controller, this.cwd)
		...
		const globalRules = await getGlobalGuardianRules(globalGuardianRulesFilePath, globalToggles, { evaluationContext })
		const globalGuardianRulesFileInstructions = globalRules.instructions

		const localRules = await getLocalGuardianRules(this.cwd, localToggles, { evaluationContext })
		const localGuardianRulesFileInstructions = localRules.instructions
		...
		const guardianIgnoreContent = this.guardianIgnoreController.guardianIgnoreContent
		let guardianIgnoreInstructions: string | undefined
		if (guardianIgnoreContent) {
			guardianIgnoreInstructions = formatResponse.guardianIgnoreInstructions(guardianIgnoreContent)
		}
		...
		const promptContext: SystemPromptContext = {
			...
			globalGuardianRulesFileInstructions,
			localGuardianRulesFileInstructions,
			localCursorRulesFileInstructions,
			localCursorRulesDirInstructions,
			localWindsurfRulesFileInstructions,
			localAgentsRulesFileInstructions,
			guardianIgnoreInstructions,
			...
			guardianWebToolsEnabled:
				this.stateManager.getGlobalSettingsKey("guardianWebToolsEnabled") && featureFlagsService.getWebtoolsEnabled(),
		...
		const contextManagementMetadata = await this.contextManager.getNewContextMessagesAndMetadata(
			...
			this.messageStateHandler.getGuardianMessages(),
			this.api,
			this.taskState.conversationHistoryDeletedRange,
			previousApiReqIndex,
			await ensureTaskDirectoryExists(this.taskId),
		...
		try {
			...
			const guardianError = ErrorService.get().toGuardianError(error, model.id, providerId)

			// Capture provider failure telemetry using guardianError
			ErrorService.get().logMessage(guardianError.message)
			...
			if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				if (isContextWindowExceededError) {
					...
					if (truncatedConversationHistory.length > 3) {
						guardianError.message = "Context window exceeded. Click retry to truncate the conversation and try again."
						this.taskState.didAutomaticallyRetryFailedApiRequest = false
					}
				}

				const streamingFailedMessage = guardianError.serialize()
				...
				if (lastApiReqStartedIndex !== -1) {
					...
					await this.messageStateHandler.updateGuardianMessage(lastApiReqStartedIndex, {
						...
						} satisfies GuardianApiReqInfo),
					})
					// this.ask will trigger postStateToWebview, so this change should be picked up.
				}

				const isAuthError = guardianError.isErrorType(GuardianErrorType.Auth)

				// Check if this is a Guardian provider insufficient credits error - don't auto-retry these
				const isGuardianProviderInsufficientCredits = (() => {
					if (providerId !== "guardian") {
						return false
					}
					try {
						const parsedError = GuardianError.transform(error, model.id, providerId)
						return parsedError.isErrorType(GuardianErrorType.Balance)
					} catch {
						return false
					}
				})()

				let response: GuardianAskResponse
				// Skip auto-retry for Guardian provider insufficient credits or auth errors
				if (!isGuardianProviderInsufficientCredits && !isAuthError && this.taskState.autoRetryAttempts < 3) {
					...
					if (autoRetryApiReqIndex !== -1) {
						const guardianMessages = this.messageStateHandler.getGuardianMessages()
						const currentApiReqInfo: GuardianApiReqInfo = JSON.parse(guardianMessages[autoRetryApiReqIndex].text || "{}")
						delete currentApiReqInfo.streamingFailedMessage
						await this.messageStateHandler.updateGuardianMessage(autoRetryApiReqIndex, {
							text: JSON.stringify(currentApiReqInfo),
						})
					}
					...
					if (!isGuardianProviderInsufficientCredits && !isAuthError) {
						await this.say(
							"error_retry",
							JSON.stringify({
								attempt: 3,
				...
				if (manualRetryApiReqIndex !== -1) {
					const guardianMessages = this.messageStateHandler.getGuardianMessages()
					const currentApiReqInfo: GuardianApiReqInfo = JSON.parse(guardianMessages[manualRetryApiReqIndex].text || "{}")
					delete currentApiReqInfo.streamingFailedMessage
					await this.messageStateHandler.updateGuardianMessage(manualRetryApiReqIndex, {
						text: JSON.stringify(currentApiReqInfo),
					})
				}
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				// Reset auto-completion timer when tool execution starts
				this.resetAutoCompletionTimer()
	...
	async recursivelyMakeGuardianRequests(userContent: GuardianContent[], includeFileDetails = false): Promise<boolean> {
		...
		if (this.taskState.consecutiveMistakeCount >= this.stateManager.getGlobalSettingsKey("maxConsecutiveMistakes")) {
			...
			if (autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Guardian is having trouble. Would you like to continue the task?",
				})
			}
			const { response, text, images, files } = await this.ask(
				"mistake_limit_reached",
				this.api.getModel().id.includes("claude")
					? `This may indicate a failure in Guardian's thought process or inability to use a tool properly, which can be mitigated with some user guidance (e.g. "Try breaking down the task into smaller steps").`
					: "Guardian uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 4.5 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				...
				const feedbackUserContent: GuardianUserContent[] = []
		...
		const previousApiReqIndex = findLastIndex(this.messageStateHandler.getGuardianMessages(), (m) => m.say === "api_req_started")

		// Save checkpoint if this is the first API request
		const isFirstRequest = this.messageStateHandler.getGuardianMessages().filter((m) => m.say === "api_req_started").length === 0
		...
		if (
			...
			try {
				...
				this.taskState.checkpointManagerErrorMessage = errorMessage // will be displayed right away since we saveGuardianMessages next which posts state to webview
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
			...
			if (lastCheckpointMessageIndex !== -1) {
				...
				commitPromise
					?.then(async (commitHash) => {
						if (commitHash) {
							await this.messageStateHandler.updateGuardianMessage(lastCheckpointMessageIndex, {
								lastCheckpointHash: commitHash,
							})
							// saveGuardianMessagesAndUpdateHistory will be called later after API response,
		...
		if (shouldCompact) {
			...
			;[parsedUserContent, environmentDetails, guardianrulesError] = await this.loadContext(
				userContent,
				includeFileDetails,
				useCompactPrompt,
			)
		...
		// error handling if the user uses the /newrule command & their .guardianrules is a file, for file read operations didnt work properly
		if (guardianrulesError === true) {
			await this.say(
				"error",
				"Issue with processing the /newrule command. Double check that, if '.guardianrules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.",
			)
		}
		...
		const lastApiReqIndex = findLastIndex(this.messageStateHandler.getGuardianMessages(), (m) => m.say === "api_req_started")
		await this.messageStateHandler.updateGuardianMessage(lastApiReqIndex, {
			text: JSON.stringify({
				request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
			} satisfies GuardianApiReqInfo),
		})
		...
		try {
			...
			const abortStream = async (cancelReason: GuardianApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			// Reset auto-completion timer when streaming starts
			this.resetAutoCompletionTimer()
			...
			const finalizePendingReasoningMessage = async (thinking: string): Promise<boolean> => {
				...
				await this.messageStateHandler.updateGuardianMessage(pendingReasoningIndex, {
					text: thinking,
					partial: false,
				})
				const completedReasoning = this.messageStateHandler.getGuardianMessages()[pendingReasoningIndex]
				if (completedReasoning) {
					await sendPartialMessageEvent(convertGuardianMessageToProto(completedReasoning))
				}
				return true
			}
			...
			try {
				for await (const chunk of stream) {
					...
					if (this.taskState.abort) {
						...
						if (!this.taskState.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of guardian)
				...
				// abandoned happens when extension is no longer waiting for the guardian instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.taskState.abandoned) {
					const guardianError = ErrorService.get().toGuardianError(error, this.api.getModel().id)
					const errorMessage = guardianError.serialize()
				...
				// Start auto-completion timer when streaming ends
				this.startAutoCompletionTimer()
			}

			// Finalize any remaining tool calls at the end of the stream

			// OpenRouter/Guardian may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantHasContent) {
				...
				const assistantContent: Array<GuardianAssistantContent> = [
				...
				if (hasAssistantText) {
					assistantContent.push({
						...
						// reasoning_details only exists for guardian/openrouter providers
			...
			if (assistantHasContent) {
				...
				const recDidEndLoop = await this.recursivelyMakeGuardianRequests(this.taskState.userMessageContent)
				...
				const baseErrorMessage =
					"Invalid API Response: The provider returned an empty or unparsable response. This is a provider-side issue where the model failed to generate valid output or returned tool calls that Guardian cannot process. Retrying the request may help resolve this issue."
	...
	async loadContext(
	...
	): Promise<[GuardianContent[], string, boolean]> {
		let needsGuardianrulesFileCheck = false
		...
		const parseTextBlock = async (text: string): Promise<string> => {
			...
			const { processedText, needsGuardianrulesFileCheck: needsCheck } = await parseSlashCommands(
				parsedText,
			...
			if (needsCheck) {
				needsGuardianrulesFileCheck = true
			}

			return processedText
		}

		const processTextContent = async (block: GuardianTextContentBlock): Promise<GuardianTextContentBlock> => {
		...
		const processContentBlock = async (block: GuardianContent): Promise<GuardianContent> => {
			if (block.type === "text") {
		...
		// Check guardianrulesData if needed
		const guardianrulesError = needsGuardianrulesFileCheck
			? await ensureLocalGuardianDirExists(this.cwd, GlobalFileNames.guardianRules)
			: false
		...
		return [processedUserContent, environmentDetails, guardianrulesError]
	}
	...
	async processNativeToolCalls(assistantTextOnly: string, toolBlocks: ToolUse[]) {
		...
		// IMPORTANT: Finalize any partial text GuardianMessage before we skip over it.
		//
		...
		//
		// Without this fix, the partial text GuardianMessage remains with partial=true. In the UI
	...
	async getEnvironmentDetails(includeFileDetails = false) {
		...
		// It could be useful for guardian to know if the user went from one or no file to another between messages, so we always include this context
		...
		// Filter paths through guardianIgnoreController
		const allowedVisibleFiles = this.guardianIgnoreController
		...
		// Filter paths through guardianIgnoreController
		const allowedOpenTabs = this.guardianIgnoreController
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(this.cwd, files, didHitLimit, this.guardianIgnoreController)
				details += result
			}
		...
		const getTotalTokensFromApiReqMessage = (msg: GuardianMessage) => {
			if (!msg.text) {
				return 0
			}
			try {
		...
		const guardianMessages = this.messageStateHandler.getGuardianMessages()
		const modifiedMessages = combineApiRequests(combineCommandSequences(guardianMessages.slice(1)))
	...
	/**
	 * Start auto-completion timer when streaming ends
	 * If LLM stops typing for 2 seconds, automatically complete the task
	 */
	private startAutoCompletionTimer(): void {
		// Clear any existing timer
		this.cancelAutoCompletionTimer()

		// Only start timer if streaming just ended and task is not aborted
		if (this.taskState.abort || this.taskState.isStreaming) {
			return
		}

		// Record when streaming ended
		this.taskState.streamingEndedAt = Date.now()

		// Start 2-second timer
		this.taskState.autoCompletionTimer = setTimeout(() => {
			this.checkAndAutoComplete()
		}, 2000)
	}

	/**
	 * Cancel the auto-completion timer
	 */
	private cancelAutoCompletionTimer(): void {
		if (this.taskState.autoCompletionTimer) {
			clearTimeout(this.taskState.autoCompletionTimer)
			this.taskState.autoCompletionTimer = undefined
		}
		this.taskState.streamingEndedAt = undefined
	}

	/**
	 * Check conditions and auto-complete the task if appropriate
	 */
	private async checkAndAutoComplete(): Promise<void> {
		// Check conditions for auto-completion
		if (
			!this.taskState.abort && // Task not aborted
			!this.taskState.isStreaming && // Not currently streaming
			!this.taskState.isWaitingForFirstChunk && // Not waiting for first chunk
			!this.taskState.didRejectTool && // No tool was rejected
			!this.taskState.didAlreadyUseTool && // No tool was just used
			!this.taskState.presentAssistantMessageLocked && // Not presenting assistant message
			!this.commandExecutor.hasActiveBackgroundCommand() && // No active background command
			!(await this.getActiveHookExecution()) // No active hook execution
		) {
			// Auto-complete the task
			await this.autoCompleteTask()
		}
	}

	/**
	 * Automatically complete the task with a generic completion message
	 */
	private async autoCompleteTask(): Promise<void> {
		try {
			// Cancel any pending timer
			this.cancelAutoCompletionTimer()

			// Use attempt_completion tool with a generic message
			// This simulates the LLM calling attempt_completion after 2 seconds of inactivity
			const completionMessage = "Task completed automatically after 2 seconds of inactivity."
			
			// We need to trigger the attempt_completion tool handler
			// Since we can't directly call the tool handler, we'll simulate it by
			// adding a completion message to the conversation
			await this.say("completion_result", completionMessage)
			
			// Also add to API conversation history for consistency
			await this.messageStateHandler.addToApiConversationHistory({
				role: "assistant",
				content: [{
					type: "text",
				...
				modelInfo: {
					providerId: this.getCurrentProviderInfo().providerId,
			...
			Logger.log(`[Task ${this.taskId}] Auto-completed task after 2 seconds of inactivity`)
		} catch (error) {
			Logger.error(`[Task ${this.taskId}] Failed to auto-complete task:`, error)
		}
	}

	/**
	 * Reset auto-completion timer when new activity is detected
	 * Call this when:
	 * - User provides input
	 * - Tool execution starts
	 * - Streaming restarts
	 * - Any other activity that should reset the 2-second timer
	 */
	public resetAutoCompletionTimer(): void {
		this.cancelAutoCompletionTimer()
		
		// Only restart timer if streaming is currently active
		if (this.taskState.isStreaming) {
			// Timer will be started when streaming ends
			return
		}
		
		// If not streaming but we want to track inactivity from now,
		// we could start the timer here, but for now we only track
		// inactivity after streaming ends
	}
}
