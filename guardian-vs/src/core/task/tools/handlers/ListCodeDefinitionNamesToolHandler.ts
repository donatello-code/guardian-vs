...
export class ListCodeDefinitionNamesToolHandler implements IFullyManagedTool {
	readonly name = GuardianDefaultTool.LIST_CODE_DEF
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const result = await parseSourceCodeForDefinitionsTopLevel(absolutePath, config.services.guardianIgnoreController)
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `Guardian wants to analyze code definitions in ${getWorkspaceBasename(absolutePath, "ListCodeDefinitionNamesToolHandler.notification")}`
