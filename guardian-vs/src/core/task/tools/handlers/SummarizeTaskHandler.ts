...
export class SummarizeTaskHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = GuardianDefaultTool.SUMMARIZE_TASK
		...
		try {
			...
			if (hooksEnabled) {
				try {
					...
					const result = await executePreCompactHookWithCleanup({
						...
						guardianMessages: config.messageState.getGuardianMessages(),
			...
			if (match) {
				...
				for (const relPath of filePaths) {
					...
					// Check .guardianignore first and skip ignored files
					...
					if (await config.callbacks.shouldAutoApproveToolWithPath(GuardianDefaultTool.FILE_READ, relPath)) {
						try {
			...
			const telemetryData = config.services.contextManager.getContextTelemetryData(
				config.messageState.getGuardianMessages(),
				config.api,
