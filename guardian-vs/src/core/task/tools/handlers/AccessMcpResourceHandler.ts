...
import { GuardianAsk, GuardianAskUseMcpServer } from "@shared/ExtensionMessage"
...
export class AccessMcpResourceHandler implements IFullyManagedTool {
	readonly name = GuardianDefaultTool.MCP_ACCESS
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		const partialMessage = JSON.stringify({
			...
			arguments: undefined,
		} satisfies GuardianAskUseMcpServer)
		...
		if (shouldAutoApprove) {
			...
			await uiHelpers.ask("use_mcp_server" as GuardianAsk, partialMessage, block.partial).catch(() => {})
		}
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!server_name) {
			...
			return await config.callbacks.sayAndCreateMissingParamError(GuardianDefaultTool.MCP_ACCESS, "server_name")
		}

		if (!uri) {
			config.taskState.consecutiveMistakeCount++
			return await config.callbacks.sayAndCreateMissingParamError(GuardianDefaultTool.MCP_ACCESS, "uri")
		}
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `Guardian wants to access ${uri || "unknown resource"} on ${server_name || "unknown server"}`
