...
export class ReportBugHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = GuardianDefaultTool.REPORT_BUG
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Guardian wants to create a github issue...",
				message: `Guardian is suggesting to create a github issue with the title: ${title}`,
			})
		}
		...
		const guardianVersion = ExtensionRegistryInfo.version
		...
		if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
			...
			try {
				...
				params.set("guardian-version", guardianVersion)
				...
				await createAndOpenGitHubIssue("guardian", "guardian", "bug_report.yml", params)
			} catch (error) {
