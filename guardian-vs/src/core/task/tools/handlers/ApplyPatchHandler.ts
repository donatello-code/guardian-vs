...
import type { GuardianSayTool } from "@shared/ExtensionMessage"
...
export const PatchGuardianSayMap = {
...
export class ApplyPatchHandler implements IFullyManagedTool {
	readonly name = GuardianDefaultTool.APPLY_PATCH
	...
	private async previewPatchStream(rawInput: string, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		await config.callbacks
			.ask(
				"tool",
				JSON.stringify({
					tool: PatchGuardianSayMap[actionType],
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			for (const changedFilePath of changedFiles) {
				...
				config.services.fileContextTracker.markFileAsEditedByGuardian(pathToTrack)
				await config.services.fileContextTracker.trackFileContext(pathToTrack, "guardian_edited")
			}
	...
	private async generateChangeSummary(changes: Record<string, FileChange>): Promise<GuardianSayTool[]> {
	...
	private async handleApproval(config: TaskConfig, block: ToolUse, message: GuardianSayTool, rawInput: string): Promise<boolean> {
		...
		showNotificationForApproval(`Guardian wants to edit '${message.path}'`, config.autoApprovalSettings.enableNotifications)
