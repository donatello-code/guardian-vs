import { BrowserAction, BrowserActionResult, browserActions, GuardianSayBrowserAction } from "@shared/ExtensionMessage"
...
export class BrowserToolHandler implements IFullyManagedTool {
	readonly name = GuardianDefaultTool.BROWSER
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		if (action === "launch") {
			...
			await uiHelpers.say(
				...
				} satisfies GuardianSayBrowserAction),
				undefined,
				undefined,
				block.partial,
			)
			...
			if (action === "launch") {
				...
				if (autoApprover.shouldAutoApproveTool(block.name)) {
					...
					showNotificationForApproval(
						`Guardian wants to use a browser and launch ${url}`,
