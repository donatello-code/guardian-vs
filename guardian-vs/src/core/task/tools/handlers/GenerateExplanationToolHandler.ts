...
import type { GuardianSayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified GuardianSayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: GuardianSayGenerateExplanation["status"],
	error?: string,
): string {
	const message: GuardianSayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = GuardianDefaultTool.GENERATE_EXPLANATION
