...
export class ToolResultUtils {
	...
	static pushToolResult(
		...
		if (typeof content === "string") {
			...
			// Get tool_use_id from map using call_id, or use "guardian" as fallback for backward compatibility
			const toolUseId = toolUseIdMap?.get(block.call_id || "") || "guardian"
			...
			// If using backward-compatible "guardian" ID and content is an array, spread it directly
			// instead of wrapping it (which would cause JSON.stringify in createToolResultBlock)
			if ((toolUseId === "guardian" || !toolUseId) && Array.isArray(content)) {
	...
	private static createToolResultBlock(content: ToolResponse, id?: string, call_id?: string) {
		// If id is "guardian", we treat it as a plain text result for backward compatibility
	 ...
	 */
	static async askApprovalAndPushFeedback(type: GuardianAsk, completeMessage: string, config: TaskConfig) {
