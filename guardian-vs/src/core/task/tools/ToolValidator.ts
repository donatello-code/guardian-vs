...
export class ToolValidator {
	constructor(private readonly guardianIgnoreController: GuardianIgnoreController) {}
	...
	/**
	 * Verifies access is allowed to a given path via .guardianignore rules.
	 * Callers should pass a repo-relative (workspace-relative) path.
	 */
	checkGuardianIgnorePath(relPath: string): ValidationResult {
		const accessAllowed = this.guardianIgnoreController.validateAccess(relPath)
		if (!accessAllowed) {
			return {
				ok: false,
				error: `Access to path '${relPath}' is blocked by .guardianignore settings.`,
			}
		}
		return { ok: true }
	}
