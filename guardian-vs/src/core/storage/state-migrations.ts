...
export async function migrateCustomInstructionsToGlobalRules(context: vscode.ExtensionContext) {
	try {
		...
		if (customInstructions?.trim()) {
			Logger.log("Migrating custom instructions to global Guardian rules...")

			// Create global .guardianrules directory if it doesn't exist
			...
			Logger.log("Successfully migrated custom instructions to global Guardian rules")
		}
	} catch (error) {
...
export async function migrateWelcomeViewCompleted(context: vscode.ExtensionContext) {
	try {
		...
		if (welcomeViewCompleted === undefined) {
			...
			const guardianAccountId = await context.secrets.get("guardianAccountId")
...
export async function cleanupOldApiKey(context: vscode.ExtensionContext) {
	try {
		...
		await context.secrets.delete("guardianApiKey")
	} catch (error) {
		Logger.error("Failed to cleanup old guardianApiKey", error)
	}
}
