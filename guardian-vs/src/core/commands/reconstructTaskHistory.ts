import { getSavedGuardianMessages, getTaskMetadata, readTaskHistoryFromState, writeTaskHistoryToState } from "@core/storage/disk"
...
async function reconstructTaskHistoryItem(taskId: string): Promise<HistoryItem | null> {
	try {
		...
		const guardianMessages = await getSavedGuardianMessages(taskId)
		...
		const taskInfo = extractTaskInformation(guardianMessages, metadata)
...
function extractTaskInformation(guardianMessages: GuardianMessage[], metadata: any): TaskInfo {
	// Find the first user message (task description)
	const firstUserMessage = guardianMessages.find((msg) => msg.type === "say" && msg.say === "text" && msg.text)

	// Extract timestamp from first message or use task ID as fallback
	const timestamp = guardianMessages.length > 0 ? guardianMessages[0].ts : Date.now()
	...
	const apiReqMessages = guardianMessages.filter(
	...
	const messageSize = JSON.stringify(guardianMessages).length
