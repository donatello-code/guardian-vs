...
import { GuardianStorageMessage, convertGuardianStorageToAnthropicMessage } from "@/shared/messages/content"

/**
 * Converts Guardian storage messages to Anthropic API format with optional cache control.
 * Adds ephemeral cache control to the last two user messages to prevent them from being
 * stored in Anthropic's cache.
 *
 * @param guardianMessages - Array of Guardian storage messages to convert
...
export function sanitizeAnthropicMessages(
	guardianMessages: Array<GuardianStorageMessage | Anthropic.MessageParam>,
	...
	const userMsgIndices = guardianMessages.reduce((acc, msg, index) => {
		if (msg.role === "user") {
			acc.push(index)
		}
		return acc
	...
	return guardianMessages.map((msg, index) => {
		const anthropicMsg = convertGuardianStorageToAnthropicMessage(msg)
