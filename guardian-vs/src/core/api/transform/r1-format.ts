...
import { GuardianAssistantThinkingBlock, GuardianStorageMessage } from "@/shared/messages/content"
...
export function addReasoningContent(
	...
	originalMessages: GuardianStorageMessage[],
		...
		if (msg.role === "assistant") {
			if (Array.isArray(msg.content)) {
				const thinking = msg.content
					.filter((p): p is GuardianAssistantThinkingBlock => p.type === "thinking")
					.map((p) => p.thinking)
					.join("\n")
				if (thinking) {
