...
export class DifyHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: GuardianStorageMessage[]): ApiStream {
		...
		const requestBody = {
			...
			user: "guardian-user", // A unique user identifier
			files: [],
		}
	...
	private convertMessagesToQuery(systemPrompt: string, messages: GuardianStorageMessage[]): string {
	 ...
	 * @param user User identifier (defaults to "guardian-user")
	 * @returns Promise with file upload response
	 */
	async uploadFile(file: Buffer, filename: string, user: string = "guardian-user"): Promise<DifyFileResponse> {
	 ...
	 */
	async stopGeneration(taskId: string, user: string = "guardian-user"): Promise<void> {
	 ...
	 */
	async deleteConversation(conversationId: string, user: string = "guardian-user"): Promise<void> {
