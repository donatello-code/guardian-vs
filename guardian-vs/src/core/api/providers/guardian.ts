...
interface GuardianHandlerOptions extends CommonApiHandlerOptions {
	ulid?: string
	taskId?: string
...
export class GuardianHandler implements ApiHandler {
	private options: GuardianHandlerOptions
	private guardianAccountService = GuardianAccountService.getInstance()
	...
	private get _baseUrl(): string {
		return GuardianEnv.config().apiBaseUrl
	}

	constructor(options: GuardianHandlerOptions) {
		this.options = options
		this._authService = AuthService.getInstance()
	}

	private async ensureClient(): Promise<OpenAI> {
		const guardianAccountAuthToken = this.options.guardianApiKey || (await this._authService.getAuthToken())
		if (!guardianAccountAuthToken) {
			throw new Error(CLINE_ACCOUNT_AUTH_ERROR_MESSAGE)
		}
		if (!this.client) {
			try {
				const defaultHeaders: Record<string, string> = {
					"HTTP-Referer": "https://guardian.js",
					"X-Title": "Guardian VS",
					"X-Task-ID": this.options.ulid || "",
				}
				Object.assign(defaultHeaders, await buildGuardianExtraHeaders())

				this.client = new OpenAI({
					baseURL: `${this._baseUrl}/api/v1`,
					apiKey: guardianAccountAuthToken,
					defaultHeaders,
				...
				throw new Error(`Error creating Guardian client: ${error.message}`)
			}
		}
		// Ensure the client is always using the latest auth token
		this.client.apiKey = guardianAccountAuthToken
		return this.client
	}

	@withRetry()
	async *createMessage(systemPrompt: string, messages: GuardianStorageMessage[], tools?: OpenAITool[]): ApiStream {
		try {
			...
			for await (const chunk of stream) {
				Logger.debug("GuardianHandler chunk:" + JSON.stringify(chunk))
				// openrouter returns an error object instead of the openai sdk throwing an error
				if ("error" in chunk) {
					const error = chunk.error as OpenRouterErrorResponse["error"]
					Logger.error(`Guardian API Error: ${error?.code} - ${error?.message}`)
					// Include metadata in the error message if available
					const metadataStr = error.metadata ? `\nMetadata: ${JSON.stringify(error.metadata, null, 2)}` : ""
					throw new Error(`Guardian API Error ${error.code}: ${error.message}${metadataStr}`)
				}
				...
				if ((choice?.finish_reason as string) === "error") {
					...
					if (choiceWithError.error) {
						...
						Logger.error(`Guardian Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
						throw new Error(`Guardian Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
					}
					throw new Error("Guardian Mid-Stream Error: Stream terminated with error status but no error details provided")
				}
			...
			if (!didOutputUsage) {
				Logger.warn("Guardian API did not return usage chunk, fetching from generation endpoint")
	...
	async getApiStreamUsage(): Promise<ApiStreamUsageChunk | undefined> {
		if (this.lastGenerationId) {
			try {
				const guardianAccountAuthToken = await this._authService.getAuthToken()
				if (!guardianAccountAuthToken) {
					throw new Error(CLINE_ACCOUNT_AUTH_ERROR_MESSAGE)
				}
				const headers: Record<string, string> = {
					// Align with backend auth expectations
					Authorization: `Bearer ${guardianAccountAuthToken}`,
				}
				Object.assign(headers, await buildGuardianExtraHeaders())

				const response = await axios.get(`${this.guardianAccountService.baseUrl}/generation?id=${this.lastGenerationId}`, {
					headers,
				...
				// ignore if fails
				Logger.error("Error fetching guardian generation details:", error)
			}
		}
		return undefined
	}
